...
(function() {
    ...
    // ==================== RECALCULATE ENTRY FUNCTION - FIXED ====================
    async function recalculateEntry(entry) {
        ...
        // Get day name from date (use noon to avoid timezone issues)
        ...
        let isHoliday = updatedEntry.is_holiday || false;
        ...
        // Calculate OT and CPL if check-in/out exists
        if (updatedEntry.check_in && updatedEntry.check_out) {
            ...
            if (hoursWorked >= 0) {
                ...
                if (isHoliday) {
                    ...
                    if (hoursWorked >= base) {
                        ...
                        window.addDebugLog(`✅ CPL earned on holiday: ${cplGrant} (worked ${hoursWorked.toFixed(2)}h >= ${base}h)`, 'success');
                    } else {
                        updatedEntry.cpl_earned = 0;
                        updatedEntry.cpl_expiry_date = null;
            ...
            // No check-in/out, zero out OT and CPL
            updatedEntry.final_ot_hours = 0;
            updatedEntry.cpl_earned = 0;
            updatedEntry.cpl_expiry_date = null;
            
            if (isHoliday) {
                window.addDebugLog('Holiday but no check-in/out - no CPL earned', 'info');
            }
        }
        
        updatedEntry.base_hours_rule = base;
        updatedEntry.ot_cap_rule = otCap;
        updatedEntry.is_holiday = isHoliday;
        updatedEntry.sync_status = 'pending';
        
        window.addDebugLog(`Recalculated - Base: ${base}, OT: ${updatedEntry.final_ot_hours}, CPL: ${updatedEntry.cpl_earned}`, 'success');
    ...
    async function getOrCreateEntry(date) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.entry) {
                    ...
                    const recalculatedEntry = await recalculateEntry(cloudEntry);
                    
                    // Save to local DB
                    await window.dbAPI.saveEntry(recalculatedEntry);
                    window.addDebugLog(`Imported entry for ${date} from cloud and recalculated`, 'success');
        ...
        // New entries are created with default values, no need to recalculate yet
        return newEntry;
    }
    ...
    async function markLeave(type) {
        ...
        // RECALCULATE before saving
        const updatedEntry = await recalculateEntry(entry);
        
        // Save to local DB
        if (window.dbAPI) {
            await window.dbAPI.saveEntry(updatedEntry);
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    // RECALCULATE before saving
                    const recalculatedEntry = await recalculateEntry(entry);
                    await window.dbAPI.saveEntry(recalculatedEntry);
                    imported++;
                }
    ...
    // ==================== CREATE MONTH END ENTRY - FIXED ====================
    async function createOrUpdateMonthEndEntry(dateStr, year, month) {
        if (!appCurrentUser || !window.dbAPI) return;
        
        window.addDebugLog(`Creating/updating month end entry for ${dateStr} (year: ${year}, month: ${month + 1})`, 'info');
        
        // Check if entry already has AL accrual for this month
        const entries = await window.dbAPI.getAllEntriesForUser(appCurrentUser.id);
        ...
        if (existingAccrual) {
            window.addDebugLog(`AL already exists for ${month + 1}/${year} - keeping existing`, 'info');
            return existingAccrual;
        }
        
        // Calculate expiry date: end of next year (Dec 31 of year+1)
        const expiryDate = calculateALExpiry(dateStr);
        ...
        if (entry) {
            ...
            const recalculatedEntry = await recalculateEntry(entry);
            await window.dbAPI.saveEntry(recalculatedEntry);
            window.addDebugLog(`✅ Added AL accrual to existing entry for ${dateStr} (expires: ${expiryDate})`, 'success');
        } else {
            // Create new accrual entry
            const accrualEntry = {
                date: dateStr,
            ...
            await window.dbAPI.saveEntry(accrualEntry);
            window.addDebugLog(`✅ Created AL accrual entry for ${dateStr} (expires: ${expiryDate})`, 'success');
        }
    }
    ...
    async function recalculateAllMonthEnds() {
        ...
        // Determine up to which month to add
        // If we're past the last day of current month, include current month
        // Otherwise, only include up to previous month
        let maxMonth = currentMonth;
        if (currentDay < lastDayOfCurrentMonth) {
            maxMonth = currentMonth - 1; // Current month not completed yet
        }
        // If it's exactly the last day, include current month (no change needed)
        
        window.addDebugLog(`Adding AL accruals from Jan to month ${maxMonth + 1}`, 'info');
        
        // Add ALL months from January to maxMonth
        for (let month = 0; month <= maxMonth; month++) {
            // Get the actual last day of the month
            const lastDay = new Date(currentYear, month + 1, 0);
            const dateStr = lastDay.toISOString().split('T')[0];
            
            window.addDebugLog(`Processing month ${month + 1}/${currentYear} - last day: ${dateStr}`, 'info');
            
            await createOrUpdateMonthEndEntry(dateStr, currentYear, month);
        }
        
        // ALWAYS add December of previous year for carry forward
        const lastYear = currentYear - 1;
        const decLastDay = new Date(lastYear, 12, 0); // December 31
        const decDateStr = decLastDay.toISOString().split('T')[0];
        
        window.addDebugLog(`Processing December ${lastYear} - last day: ${decDateStr}`, 'info');
        await createOrUpdateMonthEndEntry(decDateStr, lastYear, 11); // 11 = December
    ...
    window.addDebugLog('app.js: Loading complete - All features working with CPL recalculation', 'success');
})();