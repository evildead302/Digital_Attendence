...
(function() {
    ...
    async function loadTodayEntry() {
        ...
        try {
            ...
            if (todayEntry) {
                ...
                // Handle leave types
                if (todayEntry.al_used || todayEntry.sl_used || todayEntry.cl_used || todayEntry.cpl_used) {
                    document.getElementById('checkInDisplay').textContent = '--:--';
                    document.getElementById('checkOutDisplay').textContent = '--:--';
                    window.addDebugLog('Leave day detected - cleared displays', 'info');
                }
            } else {
    ...
    async function calculateOT(checkIn, checkOut) {
        ...
        if (dayName === 'sunday') {
            ...
            if (sundayWeek % 2 === 1) { // Odd Sundays (1st, 3rd, 5th)
                ...
                window.addDebugLog(`Sunday #${sundayWeek} (odd) - Base: ${base}, OT Cap: ${otCap}, CPL: ${cplGrant}`, 'info');
                ...
                window.addDebugLog(`Sunday #${sundayWeek} (even) - Base: ${base}, OT Cap: ${otCap}, CPL: ${cplGrant}`, 'info');
            }
        } else {
    ...
    async function syncToCloud() {
        ...
        try {
            ...
            const pendingEntries = await window.dbAPI.getEntriesNeedingSync(20); // Limit to 20 for debugging
            
            window.addDebugLog(`Found ${pendingEntries.length} pending entries`, 'info');
            ...
            // Log each pending entry
            window.addDebugLog('=== PENDING ENTRIES ===', 'info');
            pendingEntries.forEach((entry, index) => {
                window.addDebugLog(`Entry ${index + 1}: ${JSON.stringify({
                    date: entry.date,
                    ...
                    off_day: entry.is_off_day
                })}`, 'info');
            });
            
            // Clean the entries - remove unnecessary fields
            const cleanEntries = pendingEntries.map(entry => ({
                date: entry.date,
            ...
            window.addDebugLog(`Sending ${cleanEntries.length} cleaned entries to cloud`, 'info');
            window.addDebugLog(`Request payload size: ${JSON.stringify(cleanEntries).length} bytes`, 'info');
            
            // Log the first entry as sample
            if (cleanEntries.length > 0) {
                window.addDebugLog(`Sample entry: ${JSON.stringify(cleanEntries[0])}`, 'info');
            }
            ...
            window.addDebugLog(`Sync request took ${endTime - startTime}ms`, 'info');
            window.addDebugLog(`Response status: ${response.status}`, 'info');
            
            if (!response.ok) {
                const errorText = await response.text();
                window.addDebugLog(`HTTP ${response.status}: ${errorText}`, 'error');
                throw new Error(`Server error: ${response.status} - ${errorText.substring(0, 100)}`);
            }
            
            const data = await response.json();
            window.addDebugLog(`Response data: ${JSON.stringify(data)}`, 'info');
            
            if (data.success) {
                // Mark as synced in local DB
                if (data.syncedIds && data.syncedIds.length > 0) {
                    window.addDebugLog(`Marking ${data.syncedIds.length} entries as synced: ${JSON.stringify(data.syncedIds)}`, 'info');
                    const result = await window.dbAPI.markAsSynced(data.syncedIds);
                    window.addDebugLog(`Mark as synced result: ${JSON.stringify(result)}`, 'info');
                }
                
                updateLastSyncTime();
                
                let message = `✅ Synced ${data.syncedIds?.length || 0} entries`;
                if (data.errors?.length > 0) {
                    message += `\n⚠️ ${data.errors.length} errors`;
                    window.addDebugLog(`Sync errors: ${JSON.stringify(data.errors)}`, 'error');
                }
                alert(message);
                window.addDebugLog('Sync completed successfully', 'success');
            } else {
    ...
    async function syncFromCloud() {
        ...
        try {
            ...
            const endTime = Date.now();
            window.addDebugLog(`Request took ${endTime - startTime}ms`, 'info');
            window.addDebugLog(`Response status: ${response.status}`, 'info');
            ...
            const data = await response.json();
            window.addDebugLog(`Response data: ${JSON.stringify(data).substring(0, 200)}...`, 'info');
            ...
            if (data.success && data.entries) {
                ...
                data.entries.slice(0, 3).forEach((entry, i) => {
                    window.addDebugLog(`Entry ${i+1}: ${JSON.stringify(entry)}`, 'info');
                });
                ...
                window.addDebugLog('No entries in response', 'warning');
            }
    ...
    async function saveSingleDateOverride() {
        ...
        window.addDebugLog(`saveSingleDateOverride() - date: ${date}, type: ${type}, base: ${base}, ot: ${ot}, cpl: ${cpl}`, 'info');
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                ...
                if (dayName === 'sunday') {
                    ...
                    if (sundayWeek % 2 === 1) { // Odd Sundays
                        ...
                        otCap = weeklyTemplate.sundayOdd.maxOT;
                        cplGrant = weeklyTemplate.sundayOdd.cpl;
                        window.addDebugLog(`Date ${dateStr}: Sunday #${sundayWeek} (odd) - Base: ${base}, OT: ${otCap}, CPL: ${cplGrant}`, 'info');
                    } else { // Even Sundays
                        base = weeklyTemplate.sundayEven.base;
                        otCap = weeklyTemplate.sundayEven.maxOT;
                        cplGrant = weeklyTemplate.sundayEven.cpl;
                        window.addDebugLog(`Date ${dateStr}: Sunday #${sundayWeek} (even) - Base: ${base}, OT: ${otCap}, CPL: ${cplGrant}`, 'info');
                    }
                } else {
                    base = weeklyTemplate[dayName]?.base || 8;
                    otCap = weeklyTemplate[dayName]?.maxOT || 1;
