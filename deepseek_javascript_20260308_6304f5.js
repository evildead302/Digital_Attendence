...
(function() {
    ...
    async function strictOverrider(entry) {
        ...
        if (entry.is_manual_adjustment === true) {
            window.addDebugLog(`🔒 ADJUSTMENT SHIELD ACTIVE - returning entry untouched with values: AL=${entry.al_adjustment}, SL=${entry.sl_adjustment}, CL=${entry.cl_adjustment}, CPL=${entry.cpl_adjustment}, OT=${entry.ot_adjustment}`, 'success');
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.entry) {
                    ...
                    window.addDebugLog(`fetchFromCloud: Leave usage - al_used=${cloudEntry.al_used}, sl_used=${cloudEntry.sl_used}, cl_used=${cloudEntry.cl_used}, cpl_used=${cloudEntry.cpl_used}`, 'info');
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    window.addDebugLog(`syncFromCloud: Leave usage - al_used=${entry.al_used}, sl_used=${entry.sl_used}, cl_used=${entry.cl_used}, cpl_used=${entry.cpl_used}`, 'info');
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        for (const entry of sortedEntries) {
            ...
            // AL Usage - FIXED: Ensure we capture leave usage
            ...
            // CPL Usage - FIXED: Ensure we capture leave usage
    ...
    async function loadBalances() {
        ...
        for (const entry of sortedEntries) {
            ...
            // Regular usage - FIXED: Use parseFloat to ensure numbers
            if (entry.sl_used) {
    ...
    async function loadExpiryInfo() {
        ...
        window.addDebugLog(`loadExpiryInfo: Checking ${alPackets.length} AL packets for expiry`, 'info');
        ...
        for (const packet of alPackets) {
            if (packet.amount > 0 && packet.expiryDate) {
                ...
                window.addDebugLog(`AL Packet from ${packet.date}: amount=${packet.amount}, expiry=${expiryDateStr}, daysUntil=${daysUntil}`, 'debug');
        ...
        if (alExpiryDiv) {
            if (Object.keys(alByYear).length === 0) {
                ...
                window.addDebugLog('No AL expiring soon', 'info');
            } else {
                ...
                window.addDebugLog(`Displaying ${Object.keys(alByYear).length} AL expiry groups`, 'success');
            }
        }
    ...
    async function markLeave(type) {
        ...
        // FIXED: Ensure we're properly setting the leave usage
        const currentValue = parseFloat(entry[`${type}_used`]) || 0;
        entry[`${type}_used`] = currentValue + 1;
        entry.check_in = null;
        entry.check_out = null;
        ...
        window.addDebugLog(`markLeave: Setting ${type}_used to ${entry[`${type}_used`]} (was ${currentValue})`, 'info');
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            try {
                ...
                if (type === 'work') {
                    ...
                    entry.check_in = null;
                    entry.check_out = null;
                    window.addDebugLog(`saveBulkManualEntry: Setting ${type}_used to ${entry[`${type}_used`]} for ${dateStr}`, 'debug');
                }
