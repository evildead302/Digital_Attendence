...
(function() {
    ...
    async function strictOverrider(entry) {
        ...
        // Recalculate AL accrual for EVERY month-end entry (same as CPL which recalculates every time)
        ...
        if (entryDate.getDate() === lastDayOfMonth) {
            ...
            updatedEntry.al_expiry_date = calculateALExpiry(updatedEntry.date);
            window.addDebugLog(`AL accrual recalculated for ${updatedEntry.date}: ${updatedEntry.al_accrued} (${userSettings.limit_annual}/12) (expires: ${updatedEntry.al_expiry_date})`, 'info');
        } else {
