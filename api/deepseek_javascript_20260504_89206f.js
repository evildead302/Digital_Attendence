...
export default async function handler(req, res) {
    ...
    return res.status(400).json({ 
        success: false, 
        message: 'Invalid action. Use: change-password, forgot-password, reset-password, reset-data, delete-account, update-settings, get-settings, update-alarm-settings, get-alarm-settings, or export' 
    });
}
...
async function handleExportData(req, res) {
    ...
    try {
        ...
        console.log(`[API] Exporting data for user: ${user.userId}, year: ${year}`);
        ...
        // Fetch attendance entries for the specified year
        ...
        console.log(`[API] Found ${entries.length} entries for year ${targetYear}`);
        ...
        res.setHeader('Content-Disposition', `attachment; filename="attendance_${targetYear}_${user.email}.csv"`);
