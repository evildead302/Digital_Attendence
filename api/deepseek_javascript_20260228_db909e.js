...
export default async function handler(req, res) {
    ...
    const isPublicRoute = path === '/api/login' || path === '/api/register';
    ...
    try {
        ...
        // SYNC FROM CLOUD (GET)
        if (req.method === 'GET' && path === '/api/sync' && req.query.direction === 'from') {
        ...
        // SYNC TO CLOUD (POST)
        if (req.method === 'POST' && path === '/api/sync' && req.query.direction === 'to') {
        ...
        console.log(`[API] Method not allowed: ${req.method} ${path}`);
        clearTimeout(timeout);
