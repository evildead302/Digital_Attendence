// api/account.js - COMPLETE FIXED VERSION
// Removed non-existent columns (is_manual_adjustment, sync_status)
// Fixed Date object handling for timestamp fields
...
export default async function handler(req, res) {
    ...
    const handlers = {
        ...
        'sync-single': handleSyncSingleEntry
    };
...
async function handleSyncSingleEntry(req, res) {
    ...
    try {
        ...
        // FIXED: Removed sync_status from INSERT
        if (existing && existing.length > 0) {
            ...
            // FIXED: Removed sync_status column (doesn't exist in schema)
            await sql`
                INSERT INTO attendance_ledger (
                    ...
                    ${entry.ot_adjustment}, ${entry.adjustment_note}
                )
            `;
...
function formatTimestamp(value) {
    ...
    // If it's a Date object (from PostgreSQL timestamp)
    ...
    if (typeof value === 'string') {
        ...
        result = result.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
        return result;
    }
...
async function handleExportData(req, res) {
    ...
    try {
        ...
        // FIXED: Removed is_manual_adjustment and sync_status (don't exist in schema)
...
function generateCSVFromEntries(entries, user, fromDate, toDate) {
    // FIXED: Removed 'Is Manual Adjustment' and 'Sync Status' headers
    const headers = [
        'Date',
        'Day of Week',
        'Check In',
    ...
    for (const entry of entries) {
        ...
        // FIXED: Use formatTimestamp helper to safely handle Date objects
        const checkInFormatted = formatTimestamp(entry.check_in);
        const checkOutFormatted = formatTimestamp(entry.check_out);
    ...
    let totalOT = 0, totalCPL = 0, totalAnnualLeave = 0, totalSickLeave = 0;
    let totalCasualLeave = 0, totalCPLUsed = 0, totalALAccrued = 0;
    let totalALAdjustment = 0, totalCPLAdjustment = 0, totalOTAdjustment = 0;
    let daysWorked = 0;
    ...
    for (const entry of entries) {
        ...
        if (entry.check_in && !entry.is_off_day && 
            !(entry.al_used > 0 || entry.sl_used > 0 || entry.cl_used > 0)) {
            daysWorked++;
        }
    }
