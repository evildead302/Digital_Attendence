...
(function() {
    ...
    // Sync push subscription with server - runs on app load
    async function syncPushSubscription() {
        ...
        if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
            window.addDebugLog('[Push] Push not supported', 'warning');
            return;
        }
        ...
        try {
            ...
            if (!subscription && vapidPublicKey) {
                ...
                window.addDebugLog('[Push] New subscription created', 'success');
            }
            ...
            if (subscription) {
                ...
                if (response.ok) {
                    window.addDebugLog('[Push] Subscription synced with server', 'success');
                } else {
                    window.addDebugLog('[Push] Failed to sync subscription with server', 'error');
                }
            } else {
                window.addDebugLog('[Push] No subscription and cannot create (missing VAPID key)', 'warning');
            }
        } catch (error) {
            window.addDebugLog(`[Push] Sync error: ${error.message}`, 'error');
        }
    }
