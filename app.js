...
(function() {
    ...
    async function syncPushSubscription(retryCount = 0) {
        ...
        try {
            ...
            window.addDebugLog('[Push] Checking for existing service worker registration...', 'info');
            
            let registration = null;
            const registrations = await navigator.serviceWorker.getRegistrations();
            window.addDebugLog(`[Push] Found ${registrations.length} service worker registrations`, 'debug');
            
            if (registrations.length > 0) {
                registration = registrations[0];
                window.addDebugLog('[Push] Using existing service worker registration', 'success');
            } else {
                // Register service worker if not found
                window.addDebugLog('[Push] No service worker found, registering...', 'info');
                try {
                    registration = await navigator.serviceWorker.register('/service-worker.js');
                    window.addDebugLog('[Push] Service worker registered successfully', 'success');
                } catch (regError) {
                    window.addDebugLog(`[Push] Service worker registration failed: ${regError.message}`, 'error');
                    localStorage.setItem('push_subscription_status', 'sw_registration_failed');
                    return false;
                }
            }
