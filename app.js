// ==================== APP.JS - ULTRA DEBUG VERSION WITH OTP ====================
(function() {
    ...
    async function strictOverrider(entry) {
        ...
        if (entry.is_manual_adjustment === true) {
            ...
            if (cplAdjustment > 0 && !entry.cpl_expiry_date) {
        ...
        if (cplAdjustment > 0 && !updatedEntry.cpl_expiry_date) {
            updatedEntry.cpl_expiry_date = calculateCPLExpiry(updatedEntry.date);
        ...
        // ===== STEP 9: AL Accrual - Only add if this is the last day of month =====
        ...
        if (entryDate.getDate() === lastDayOfMonth && !updatedEntry.al_accrued) {
            ...
            updatedEntry.al_expiry_date = calculateALExpiry(updatedEntry.date);
            window.addDebugLog(`Added AL accrual: 1.833 for month end (expires: ${updatedEntry.al_expiry_date})`, 'info');
        }
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        for (const entry of sortedEntries) {
            ...
            // CPL Usage - Capture leave usage
            if (entry.cpl_used && parseFloat(entry.cpl_used) > 0) {
        ...
        for (const packet of alPacketsCopy) {
            if (packet.amount > 0) {
                ...
                if (!expiryDate || expiryDate > targetDate) {
                    ...
                    window.addDebugLog(`FIFO: Remaining AL packet from ${packet.date}: ${packet.amount} (year ${year})`, 'debug');
                }
            }
        }
        ...
        for (const [year, amount] of Object.entries(remainingByYear)) {
            ...
            if (yearEndCaps[yearNum] !== undefined && amount > yearEndCaps[yearNum]) {
                ...
                window.addDebugLog(`FIFO: Year ${year} amount ${amount} capped to ${yearEndCaps[yearNum]}`, 'debug');
            } else {
                alBalance += amount;
            }
        }
        ...
        window.addDebugLog(`FIFO Matchmaker complete - AL: ${alBalance.toFixed(2)} (after caps), CPL: ${cplBalance.toFixed(2)}, OT Adj: ${totalOTAdjustment}, SL Adj: ${totalSLAdjustment}, CL Adj: ${totalCLAdjustment}`, 'success');
    ...
    async function loadBalances() {
        ...
        // Calculate OT totals from regular work AND adjustments
        const now = new Date();
        ...
        for (const entry of sortedEntries) {
            ...
            // OT totals - count both regular OT and OT adjustments
        ...
        // Add OT adjustments to totals - need to track by month based on entry date
        ...
        // Add SL/CL adjustments to balances
        ...
        window.addDebugLog(`Balances - AL: ${fifoResult.alBalance.toFixed(2)} (capped), SL: ${slBalance.toFixed(2)}, CL: ${clBalance.toFixed(2)}, CPL: ${fifoResult.cplBalance.toFixed(2)}, OT: ${totalOT.toFixed(1)} (with adjustments)`, 'success');
    }
    ...
    async function loadExpiryInfo() {
        ...
        // ===== CPL EXPIRY - Show CPL packets expiring within 180 days, sorted by nearest expiry =====
        const cplExpiring = [];
        ...
        // Alternative calculation: balance - 22 (if balance > 22)
        const alBalance = parseFloat(document.getElementById('alBalance').textContent) || 0;
        const alExpiringFromBalance = Math.max(0, alBalance - 22);
        
        // Use the packet-based calculation as it's more accurate
        const alExpiringTotal = alExpiringThisYear;
        
        window.addDebugLog(`AL Expiring this year: ${alExpiringTotal.toFixed(2)} (from packets: ${alExpiringThisYear.toFixed(2)}, from balance-22: ${alExpiringFromBalance.toFixed(2)})`, 'info');
            ...
            if (alExpiringTotal <= 0) {
                ...
                alExpiryDiv.innerHTML = `
                    ...
                    <div class="expiry-item">
                        ...
                        <div>(Balance above 22-day carryover limit)</div>
                    </div>
                `;
    ...
    function showForgotPassword() {
        window.addDebugLog('========== SHOW FORGOT PASSWORD CALLED ==========', 'info');
    ...
    async function initiateForgotPassword(email) {
        ...
        try {
            window.addDebugLog(`Sending request to /api/account?action=forgot-password`, 'info');
            ...
            if (data.success) {
                if (data.requiresVerification) {
                    window.addDebugLog(`Password reset requires OTP verification for ${email}`, 'success');
                    ...
                    pendingEmail = email;
                    window.addDebugLog(`Calling showOTPModal with purpose 'reset'`, 'info');
                    showOTPModal({
    ...
    // ==================== UPDATED RESET PASSWORD MODAL FUNCTIONS ====================
    ...
    function showResetPasswordModal(email) {
        ...
        // Create modal HTML with inline styles to ensure it displays
        const modalHtml = `
            <div id="resetPasswordModal" class="modal" style="display: flex; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 10001; align-items: center; justify-content: center;">
                <div class="modal-content" style="background: white; border-radius: 12px; padding: 24px; max-width: 400px; width: 90%; box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
                    <h3 style="margin: 0 0 8px 0; color: #333;">Reset Password</h3>
                    <p style="color: #666; margin-bottom: 20px; font-size: 14px;">Set a new password for<br><strong>${email}</strong></p>
                    
                    <div class="input-group" style="margin-bottom: 16px;">
                        <label style="display: block; margin-bottom: 5px; color: #555; font-size: 14px;">New Password</label>
                        <input type="password" id="resetNewPassword" class="modal-input" placeholder="Min 6 characters" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px;">
                    </div>
                    
                    <div class="input-group" style="margin-bottom: 16px;">
                        <label style="display: block; margin-bottom: 5px; color: #555; font-size: 14px;">Confirm Password</label>
                        <input type="password" id="resetConfirmPassword" class="modal-input" placeholder="Re-enter new password" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px;">
                    </div>
                    
                    <div id="resetError" class="error-message" style="color: #e74c3c; font-size: 12px; margin-bottom: 16px; min-height: 20px;"></div>
                    
                    <div class="modal-buttons" style="display: flex; gap: 12px; justify-content: flex-end;">
                        <button onclick="closeResetModal()" class="modal-cancel" style="padding: 10px 20px; background: #e0e0e0; border: none; border-radius: 6px; cursor: pointer;">Cancel</button>
                        <button onclick="submitPasswordReset('${email}')" class="modal-save" style="padding: 10px 20px; background: #4CAF50; color: white; border: none; border-radius: 6px; cursor: pointer;">Reset Password</button>
                    </div>
                </div>
            </div>
        `;
        ...
        if (modal) {
            window.addDebugLog(`✅ Reset password modal successfully added to DOM`, 'success');
            window.addDebugLog(`Modal display style: ${modal.style.display}`, 'debug');
        } else {
            window.addDebugLog(`❌ ERROR: Reset password modal not found in DOM after insertion`, 'error');
        }
    ...
    async function submitPasswordReset(email) {
        window.addDebugLog(`========== SUBMIT PASSWORD RESET START ==========`, 'info');
        window.addDebugLog(`Email: ${email}`, 'info');
        
        const newPassword = document.getElementById('resetNewPassword')?.value;
        const confirmPassword = document.getElementById('resetConfirmPassword')?.value;
        
        window.addDebugLog(`New password length: ${newPassword?.length || 0}`, 'debug');
        window.addDebugLog(`Confirm password length: ${confirmPassword?.length || 0}`, 'debug');
        ...
        if (submitBtn) {
            ...
            window.addDebugLog('Submit button disabled', 'debug');
        }
        
        try {
            window.addDebugLog(`Sending password reset request to /api/account?action=reset-password`, 'info');
            ...
            if (data.success) {
                ...
                if (submitBtn) {
                    ...
                    window.addDebugLog('Submit button re-enabled', 'debug');
                }
            }
        } catch (error) {
            ...
            if (submitBtn) {
                submitBtn.disabled = false;
                submitBtn.textContent = 'Reset Password';
                window.addDebugLog('Submit button re-enabled after error', 'debug');
            }
        }
        window.addDebugLog(`========== SUBMIT PASSWORD RESET END ==========`, 'info');
    }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.requiresVerification && data.otpCode) {
                    ...
                    pendingEmail = email;
                    window.addDebugLog(`OTP modal triggered for registration verification`, 'success');
                } else {
    ...
    function displayHistory(entries) {
        ...
        entries.forEach(e => {
            ...
            if (e.cpl_used) totalLeave += parseFloat(e.cpl_used) || 0;
        });
        ...
        entries.slice(0, 50).forEach(e => {
            ...
            } else if (e.cpl_used && parseFloat(e.cpl_used) > 0) {
            ...
            if (e.ot_adjustment && e.ot_adjustment !== 0) {
    ...
    async function saveBalanceAdjustment() {
        ...
        if (sl !== 0) {
            const oldValue = entry.sl_adjustment;
            entry.sl_adjustment = (entry.sl_adjustment || 0) + sl;
            window.addDebugLog(`STEP 6b1: SL adjustment changed from ${oldValue} to ${entry.sl_adjustment} (added ${sl})`, 'success');
        }
        
        if (cl !== 0) {
            const oldValue = entry.cl_adjustment;
            entry.cl_adjustment = (entry.cl_adjustment || 0) + cl;
            window.addDebugLog(`STEP 6b2: CL adjustment changed from ${oldValue} to ${entry.cl_adjustment} (added ${cl})`, 'success');
        }
    ...
    async function loadAdjustments() {
        ...
        adjustments.slice(0, 20).forEach(adj => {
            ...
            if (cplValue !== 0) details.push(`CPL: ${cplValue > 0 ? '+' : ''}${cplValue.toFixed(2)}`);
            if (otValue !== 0) details.push(`OT: ${otValue > 0 ? '+' : ''}${otValue.toFixed(1)}`);
    ...
    window.addDebugLog('app.js: Loading complete - ULTRA DEBUG VERSION WITH OTP - Endpoints updated to merged API structure', 'success');
})();
