...
(function() {
    ...
    // Calculate checkout time based on entry's base hours and OT
    function calculateCheckoutTimeFromEntry(entry, checkinTime) {
        ...
        const otHours = entry.final_ot_hours || 0;
    ...
    async function recalculateCheckoutAlarm(date) {
        ...
        const checkoutTime = new Date(checkinTime.getTime() + (totalWorkHours * 60 * 60 * 1000));
        const checkoutTimeLocal = `${String(checkoutTime.getHours()).padStart(2, '0')}:${String(checkoutTime.getMinutes()).padStart(2, '0')}`;
        
        window.addDebugLog(`[Alarm] Recalculating checkout alarm for ${date}: base=${baseHours}, OT=${otHours}, total=${totalWorkHours}h, new checkout=${checkoutTimeLocal}`, 'info');
        
        // Update checkout alarm in database
    ...
    function getLocalTimeForDB(date = new Date()) {
        ...
        // Return ISO-like string but with local time values (no UTC conversion)
    ...
    async function loadAlarmSettings() {
        ...
        // Step 1: Pre-download timezone sync - update server with current mobile timezone offset
        const currentTzOffset = new Date().getTimezoneOffset();
        try {
            // Send current timezone to server before fetching settings
            await fetch('/api/notifications?action=sync-timezone', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${appAuthToken}`,
                    'Content-Type': 'application/json'
            ...
            window.addDebugLog('[Alarm] Timezone synced with server: ' + currentTzOffset, 'info');
        } catch (syncError) {
            window.addDebugLog('[Alarm] Timezone sync failed: ' + syncError.message, 'warning');
        }
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                if (data.success) {
                    // Step 2: UTC-to-Local Import Logic with offset difference calculation
                    ...
                    // Calculate difference between cloud offset and current mobile offset
                    const offsetDiff = currentTzOffset - cloudTzOffset;
                    const offsetDiffHours = offsetDiff / 60;
                    
                    window.addDebugLog(`[Alarm] Cloud offset: ${cloudTzOffset}, Current offset: ${currentTzOffset}, Diff: ${offsetDiffHours}h`, 'info');
                    
                    // Convert checkin time from cloud UTC to local time using offset difference
                    let localCheckinTime = cloudCheckinTime;
                    if (cloudCheckinTime && offsetDiff !== 0) {
                        const [hours, minutes] = cloudCheckinTime.split(':').map(Number);
                        let localHours = hours + offsetDiffHours;
                        if (localHours >= 24) localHours -= 24;
                        if (localHours < 0) localHours += 24;
                        localCheckinTime = `${String(Math.floor(localHours)).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`;
                    }
                    
                    // Convert checkout time from cloud UTC to local time using offset difference
                    let localCheckoutTime = cloudCheckoutTime;
                    if (cloudCheckoutTime && offsetDiff !== 0) {
                        const [hours, minutes] = cloudCheckoutTime.split(':').map(Number);
                        let localHours = hours + offsetDiffHours;
                        if (localHours >= 24) localHours -= 24;
                        if (localHours < 0) localHours += 24;
                        localCheckoutTime = `${String(Math.floor(localHours)).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`;
                    }
                    ...
                    currentAlarmSettings = {
                        ...
                        tzOffset: currentTzOffset  // Store current offset, not cloud offset
                    };
                    
                    // Update UI elements with corrected local times
                    ...
                    window.addDebugLog('[Alarm] Settings loaded and converted to local time:', currentAlarmSettings);
                }
            }
        } catch (error) {
    ...
    async function saveAlarmSettings() {
        ...
        // Step 3: Dynamic OT based on Daily Schedule
        // Get current day of week (0 = Sunday, 1 = Monday, ..., 6 = Saturday)
        const today = new Date();
        const dayOfWeek = today.getDay();
        
        // Map JavaScript day to our weeklyTemplate keys
        const dayMapping = {
            0: 'sundayOdd',  // Sunday - will determine odd/even later
            1: 'monday',
            2: 'tuesday',
            3: 'wednesday',
            4: 'thursday',
        ...
        if (dayOfWeek === 0) {
            // Sunday - need to determine odd/even based on week number
            ...
            baseHours = weeklyTemplate[dayKey]?.base || 8;
            maxOT = weeklyTemplate[dayKey]?.maxOT || 1;
        }
        ...
        const defaultCheckoutTime = `${String(checkoutHour).padStart(2, '0')}:${String(checkoutMinute).padStart(2, '0')}`;
        
        window.addDebugLog(`[Alarm] Saving: enabled=${isEnabled}, checkin=${checkinTime}, checkout=${defaultCheckoutTime}, offset=${tzOffset}`, 'info');
        window.addDebugLog(`[Alarm] Day calculation: Day=${dayOfWeek}, Base=${baseHours}h, OT Cap=${maxOT}h, Total=${totalWorkHours}h`, 'info');
        ...
        try {
            const response = await fetch('/api/notifications?action=save', {
                ...
                body: JSON.stringify({
                    ...
                    checkout_time_local: defaultCheckoutTime,
                    tz_offset: tzOffset
                })
            });
            ...
            if (data.success) {
                ...
                // Update UI display
                const alarmCheckoutDisplay = document.getElementById('alarmCheckoutDisplay');
                if (alarmCheckoutDisplay) alarmCheckoutDisplay.textContent = defaultCheckoutTime;
                
                // Update user object in localStorage
                if (appCurrentUser) {
                    appCurrentUser.is_alarm_enabled = isEnabled;
                    appCurrentUser.default_checkin_time = checkinTime;
                    appCurrentUser.default_checkout_time = defaultCheckoutTime;
    ...
    async function updateTargetTimeDisplay() {
        ...
        if (todayEntry?.check_in && !todayEntry?.check_out) {
            // User is checked in - calculate based on actual check-in time (local time)
            ...
            calculationDisplay.innerHTML = `📊 Expected: ${formatTimeDisplay(checkinTime)} + ${baseHours}h Base + ${otHours}h OT = ${targetTimeStr}`;
            ...
            if (alarmTime && currentAlarmSettings.enabled) {
                ...
                targetDisplay.textContent = targetTimeStr;
                calculationDisplay.innerHTML = `📅 Planned: ${checkinDisplay} + ${baseHours}h Base + ${otHours}h OT = ${targetTimeStr}`;
    ...
    async function checkIn() {
        ...
        // First save to get calculated values (base_hours_rule, final_ot_hours)
        ...
        try {
            ...
            if (entry) {
                ...
                if (isLeaveDay || entry.is_off_day || entry.is_manual_adjustment) {
                    ...
                    if (entry.check_in) {
                        // Parse local time from stored value
    ...
    window.addDebugLog('app.js: Loading complete - ULTRA DEBUG VERSION WITH NOTIFICATION SCHEDULER & ALARMS', 'success');
})();
