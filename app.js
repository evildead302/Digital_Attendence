// ==================== APP.JS - ADJUSTMENT SHIELD & DYNAMIC HOLIDAYS ====================
(function() {
    ...
    function determineIsHoliday(entry, dayName, sundayWeek) {
        // A: Is it a Sunday?
        if (dayName === 'sunday') {
            return true;
        }
        ...
        // C: Is it a public holiday from template? (Sundays already covered)
        // For non-Sundays, check if template marks it as holiday (could be custom)
        if (weeklyTemplate[dayName]?.isHoliday) {
            return true;
        }
    ...
    async function strictOverrider(entry) {
        ...
        window.addDebugLog(`STRICT OVERRIDER: Processing ${entry.date} (is_manual_adjustment: ${entry.is_manual_adjustment})`, 'info');
        
        // Make a copy to work with
        const updatedEntry = { ...entry };
        
        // ===== ADJUSTMENT SHIELD =====
        // If this is a manual adjustment, ONLY touch adjustment columns
        if (updatedEntry.is_manual_adjustment === true) {
            window.addDebugLog('Adjustment Shield active - preserving work columns', 'info');
            
            // Ensure work-related columns remain NULL
            updatedEntry.check_in = null;
            updatedEntry.check_out = null;
            updatedEntry.base_hours_rule = null;
            updatedEntry.ot_cap_rule = null;
            updatedEntry.cpl_grant_rule = null;
            updatedEntry.final_ot_hours = 0;
            updatedEntry.cpl_earned = 0;
            updatedEntry.cpl_expiry_date = null;
            updatedEntry.is_off_day = false;
            updatedEntry.is_holiday = false;
            
            // ALREADY have adjustment values from save function
            // Just ensure sync status
            updatedEntry.sync_status = 'pending';
            
            window.addDebugLog(`Adjustment Shield complete - AL: ${updatedEntry.al_adjustment}, CPL: ${updatedEntry.cpl_adjustment}`, 'success');
            return updatedEntry;
        }
        
        // ===== NOT A MANUAL ADJUSTMENT - PROCEED WITH FULL OVERRIDE =====
        ...
        updatedEntry.is_holiday = determineIsHoliday(updatedEntry, dayName, sundayWeek);
        ...
        // ===== STEP 5: STRICT OT OVERWRITE (always calculated) =====
        ...
        // ===== STEP 6: CPL Calculation with Expiry =====
        if (updatedEntry.is_holiday && hoursWorked >= updatedEntry.base_hours_rule && updatedEntry.cpl_grant_rule > 0) {
    ...
    // ==================== ADVANCED FIFO MATCHMAKER WITH ADJUSTMENT PACKETS ====================
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        // APPLY YEAR-END CAP (22-day limit) - Calculate Dec 31st balances
        ...
        for (let i = 0; i < years.length; i++) {
            ...
            if (dec31Balance > 22) {
                ...
                jan1Balances[nextYear] = dec31Balance;
            }
        }
        ...
        for (const [year, amount] of Object.entries(remainingByYear)) {
            ...
            // If this year's remaining amount exceeds the cap, apply the cap
            if (yearEndCaps[yearNum] !== undefined && amount > yearEndCaps[yearNum]) {
                alBalance += yearEndCaps[yearNum];
                window.addDebugLog(`Year ${year} capped from ${amount.toFixed(2)} to ${yearEndCaps[yearNum].toFixed(2)}`, 'info');
            } else {
                alBalance += amount;
            }
        }
        ...
        try {
            ...
            for (let i = 0; i < pendingEntries.length; i++) {
                ...
                const cleanEntry = {
                    ...
                    base_hours_rule: entry.base_hours_rule !== null ? entry.base_hours_rule : null,
                    ot_cap_rule: entry.ot_cap_rule !== null ? entry.ot_cap_rule : null,
                    cpl_grant_rule: entry.cpl_grant_rule !== null ? entry.cpl_grant_rule : null,
    ...
    window.addDebugLog('app.js: Loading complete - ADJUSTMENT SHIELD ACTIVE', 'success');
})();
