...
(function() {
    ...
    async function strictOverrider(entry) {
        ...
        // ===== STEP 9: AL Accrual - DYNAMIC based on userSettings.limit_annual =====
        // ONLY recalculate AL accrual if this entry is being actively edited
        // Check if this entry is being modified (check_in, check_out, base_hours_rule, etc. changed)
        // OR if it's a new entry (al_accrued is null/undefined or 0 and it's month end)
        const isBeingEdited = (entry.check_in !== updatedEntry.check_in) ||
                              (entry.check_out !== updatedEntry.check_out) ||
                              (entry.base_hours_rule !== updatedEntry.base_hours_rule) ||
                              (entry.ot_cap_rule !== updatedEntry.ot_cap_rule) ||
                              (entry.cpl_grant_rule !== updatedEntry.cpl_grant_rule) ||
                              (entry.al_used !== updatedEntry.al_used) ||
                              (entry.sl_used !== updatedEntry.sl_used) ||
                              (entry.cl_used !== updatedEntry.cl_used) ||
                              (entry.cpl_used !== updatedEntry.cpl_used) ||
                              (entry.is_off_day !== updatedEntry.is_off_day) ||
                              (entry.al_accrued === undefined || entry.al_accrued === null || entry.al_accrued === 0);
        
        const lastDayOfMonth = new Date(entryDate.getFullYear(), entryDate.getMonth() + 1, 0).getDate();
        
        if (entryDate.getDate() === lastDayOfMonth && isBeingEdited) {
            ...
            updatedEntry.al_expiry_date = calculateALExpiry(updatedEntry.date);
            window.addDebugLog(`AL accrual calculated for edited entry ${updatedEntry.date}: ${updatedEntry.al_accrued} (${userSettings.limit_annual}/12) (expires: ${updatedEntry.al_expiry_date})`, 'info');
        } else if (entryDate.getDate() === lastDayOfMonth && updatedEntry.al_accrued && updatedEntry.al_accrued > 0) {
            // Keep existing accrual for month-end entries that are not being edited
            window.addDebugLog(`Keeping existing AL accrual for ${updatedEntry.date}: ${updatedEntry.al_accrued} (not being edited)`, 'debug');
        } else if (entryDate.getDate() !== lastDayOfMonth) {
            // For non-month-end entries, ensure no accrual is set
