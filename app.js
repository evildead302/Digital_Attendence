...
(function() {
    ...
    async function calculateOT(checkIn, checkOut, existingEntry = {}) {
        ...
        // Then check if it's a Sunday (ALL Sundays are holidays)
            ...
            if (sundayWeek % 2 === 1) { // Odd Sundays (1st, 3rd, 5th)
                ...
                window.addDebugLog(`Sunday #${sundayWeek} (odd - Holiday) - Base: ${base}, OT Cap: ${otCap}, CPL: ${cplGrant}`, 'info');
                ...
                window.addDebugLog(`Sunday #${sundayWeek} (even - Holiday) - Base: ${base}, OT Cap: ${otCap}, CPL: ${cplGrant}`, 'info');
            }
        }
        // Then check if explicitly marked as holiday
        else if (existingEntry.is_holiday) {
            window.addDebugLog('Explicit holiday', 'info');
            isHoliday = true;
        ...
        // IMPORTANT: Check for undefined, but allow 0 as a valid value
        ...
        const entry = {
            ...
            cpl_earned: isHoliday ? cplGrant : 0, // Only grant CPL on holidays
            ...
            is_holiday: isHoliday || existingEntryFromDB.is_holiday || false,
            sync_status: 'pending'
        };
    ...
    async function saveManualEntry() {
        ...
        if (type === 'work') {
            ...
            if (checkIn && checkOut) {
                ...
                entry.cpl_earned = isHoliday ? cplGrant : 0;
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    // FIX: Convert string numbers to actual numbers (allow 0)
    ...
    async function saveSingleDateOverride() {
        ...
        window.addDebugLog(`base: ${baseInput}, ot: ${otInput}, cpl: ${cplInput}`, 'info');
        ...
        if (type === 'work') {
            ...
            // Allow 0 as valid value - check if input is not empty string
            ...
            entry.is_holiday = true;
            entry.is_off_day = false;
            // For holidays, set CPL grant if provided (allow 0), otherwise use default
            if (cplInput !== '') {
                entry.cpl_grant_rule = parseFloat(cplInput);
            } else if (existingEntry.cpl_grant_rule === undefined) {
                entry.cpl_grant_rule = 1.0; // Default holiday CPL only if not set
            }
            // Allow base hours and OT cap if provided (allow 0)
            if (baseInput !== '') entry.base_hours_rule = parseFloat(baseInput);
            if (otInput !== '') entry.ot_cap_rule = parseFloat(otInput);
            ...
            entry.is_off_day = true;
            entry.is_holiday = false;
            // Clear work-related fields for off day (set to 0 explicitly)
        ...
        try {
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                ...
                if (dayName === 'sunday') {
                    ...
                    if (sundayWeek % 2 === 1) { // Odd Sundays - 1.0 CPL
                        base = weeklyTemplate.sundayOdd.base;
                        otCap = weeklyTemplate.sundayOdd.maxOT;
                        cplGrant = weeklyTemplate.sundayOdd.cpl;
                        window.addDebugLog(`Date ${dateStr}: Sunday #${sundayWeek} (odd - HOLIDAY) - Base: ${base}, OT: ${otCap}, CPL: ${cplGrant}`, 'info');
                    } else { // Even Sundays - 0.5 CPL
                        base = weeklyTemplate.sundayEven.base;
                        otCap = weeklyTemplate.sundayEven.maxOT;
                        cplGrant = weeklyTemplate.sundayEven.cpl;
                        window.addDebugLog(`Date ${dateStr}: Sunday #${sundayWeek} (even - HOLIDAY) - Base: ${base}, OT: ${otCap}, CPL: ${cplGrant}`, 'info');
                    }
                } else {
                    base = weeklyTemplate[dayName]?.base || 8;
                    otCap = weeklyTemplate[dayName]?.maxOT || 1;
