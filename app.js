...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Setup OT toggle button listener
        ...
        // Setup CPL toggle button listener
    ...
    async function exportAttendanceToCSV() {
        ...
        try {
            const response = await fetch(`/api/export?from=${fromDate}&to=${toDate}`, {
                method: 'GET',
    ...
    // ==================== SINGLE ENTRY SYNC (NEW FUNCTION) ====================
    async function syncSingleEntryToCloud(entry) {
        ...
        try {
            ...
            const response = await fetch('/api/sync?direction=single', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${appAuthToken}`,
                    'Content-Type': 'application/json'
    ...
    function updateSettingsUI() {
        // Update the toggle buttons instead of checkboxes (removed duplicate feature toggles)
        updateOCToggleButtons();
        
        // Update limit displays if they exist elsewhere
        const annualLimitDisplay = document.getElementById('userLimitAnnual');
        const casualLimitDisplay = document.getElementById('userLimitCasual');
        const sickLimitDisplay = document.getElementById('userLimitSick');
        
        if (annualLimitDisplay) annualLimitDisplay.value = userSettings.limit_annual;
        if (casualLimitDisplay) casualLimitDisplay.value = userSettings.limit_casual;
        if (sickLimitDisplay) sickLimitDisplay.value = userSettings.limit_sick;
    }
    ...
    async function saveUserSettingsAndApply() {
        ...
        // Get values from toggle buttons (which reflect userSettings)
        // The toggle buttons already update userSettings when clicked
        // So we just need to save what's already in userSettings
        
        window.addDebugLog(`Saving settings: ${JSON.stringify(userSettings)}`, 'info');
    ...
    async function saveAlarmSettings() {
        ...
        // FIXED: Correct backtick syntax - was missing backtick before ', 'debug'
