// ==================== APP.JS - ABSOLUTE ADJUSTMENT SHIELD ====================
(function() {
    ...
    async function strictOverrider(entry) {
        ...
        if (entry.is_manual_adjustment === true) {
            window.addDebugLog(`Adjustment Shield ACTIVE - preserving adjustment values: AL=${entry.al_adjustment}, CPL=${entry.cpl_adjustment}`, 'success');
        ...
        const cappedOT = Math.min(Math.max(rawOT, 0), updatedEntry.ot_cap_rule);
    ...
    async function fetchFromCloud(date) {
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.entry) {
                    ...
                    // Ensure adjustment fields exist
                    cloudEntry.al_adjustment = cloudEntry.al_adjustment !== undefined ? cloudEntry.al_adjustment : 0;
                    cloudEntry.cpl_adjustment = cloudEntry.cpl_adjustment !== undefined ? cloudEntry.cpl_adjustment : 0;
                    cloudEntry.ot_adjustment = cloudEntry.ot_adjustment !== undefined ? cloudEntry.ot_adjustment : 0;
                    
                    window.addDebugLog(`Cloud entry loaded with adjustments: AL=${cloudEntry.al_adjustment}, CPL=${cloudEntry.cpl_adjustment}`, 'info');
        ...
        // Create new entry - ALL fields defined
        const newEntry = {
            date: date,
        ...
        try {
            ...
            for (let i = 0; i < pendingEntries.length; i++) {
                ...
                const cleanEntry = {
                    ...
                    is_manual_adjustment: entry.is_manual_adjustment || false,
                ...
                window.addDebugLog(`Syncing entry ${entry.date} with adjustments: AL=${cleanEntry.al_adjustment}, CPL=${cleanEntry.cpl_adjustment}`, 'info');
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    window.addDebugLog(`Cloud entry ${entry.date} has adjustments: AL=${entry.al_adjustment}, CPL=${entry.cpl_adjustment}`, 'info');
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        for (let i = 0; i < years.length; i++) {
            ...
            // Calculate Dec 31st balance for this year
    ...
    // ==================== BALANCE ADJUSTMENT FUNCTIONS - FIXED ====================
    ...
    async function saveBalanceAdjustment() {
        ...
        window.addDebugLog(`saveBalanceAdjustment called with date: ${date}, al: ${al}, cpl: ${cpl}, ot: ${ot}`, 'info');
        ...
        // Log the entry before modification
        window.addDebugLog(`BEFORE adjustment - Entry: ${JSON.stringify({
            date: entry.date,
        ...
        // Clear all work-related fields to NULL
        entry.check_in = null;
        entry.check_out = null;
        entry.base_hours_rule = null;
        entry.ot_cap_rule = null;
        ...
        // IMPORTANT: Ensure adjustment fields exist
        ...
        if (al !== 0) {
            ...
            window.addDebugLog(`Set al_adjustment to: ${entry.al_adjustment}`, 'success');
            ...
            if (entry.al_adjustment > 0 && !entry.al_expiry_date) {
                ...
                window.addDebugLog(`Set al_expiry_date to: ${entry.al_expiry_date}`, 'info');
            }
        }
        
        if (cpl !== 0) {
            entry.cpl_adjustment = (entry.cpl_adjustment || 0) + cpl;
            window.addDebugLog(`Set cpl_adjustment to: ${entry.cpl_adjustment}`, 'success');
            
            if (entry.cpl_adjustment > 0 && !entry.cpl_expiry_date) {
                entry.cpl_expiry_date = calculateCPLExpiry(date);
                window.addDebugLog(`Set cpl_expiry_date to: ${entry.cpl_expiry_date}`, 'info');
            }
        }
        
        if (ot !== 0) {
            entry.ot_adjustment = (entry.ot_adjustment || 0) + ot;
            window.addDebugLog(`Set ot_adjustment to: ${entry.ot_adjustment}`, 'success');
        }
        
        entry.adjustment_note = note;
        
        // Log the entry after modification
        window.addDebugLog(`AFTER adjustment - Entry: ${JSON.stringify({
            date: entry.date,
            al_adjustment: entry.al_adjustment,
            cpl_adjustment: entry.cpl_adjustment,
            ot_adjustment: entry.ot_adjustment,
        ...
        // Save directly to DB first to ensure values are stored
        if (window.dbAPI) {
            window.addDebugLog('Saving directly to local DB', 'info');
            await window.dbAPI.saveEntry(entry);
            window.addDebugLog('Direct save complete', 'success');
        }
        
        // Also run through saveAndSync to handle sync
        window.addDebugLog('Calling saveAndSync', 'info');
        await saveAndSync(entry);
    ...
    async function loadAdjustments() {
        ...
        const entries = await window.dbAPI.getAllEntriesForUser(appCurrentUser.id);
        window.addDebugLog(`Found ${entries.length} total entries`, 'info');
        ...
        window.addDebugLog(`Found ${adjustments.length} adjustment entries`, 'info');
        ...
        adjustments.slice(0, 20).forEach(adj => {
            window.addDebugLog(`Displaying adjustment: ${adj.date} - AL:${adj.al_adjustment}, CPL:${adj.cpl_adjustment}, OT:${adj.ot_adjustment}`, 'info');
    ...
    window.addDebugLog('app.js: Loading complete - ABSOLUTE ADJUSTMENT SHIELD ACTIVE', 'success');
})();
