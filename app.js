...
(function() {
    ...
    async function fetchOrCreateEntry(date) {
        ...
        if (entry) {
            ...
            window.addDebugLog(`Local entry data - al_adjustment:${entry.al_adjustment}, cpl_adjustment:${entry.cpl_adjustment}, ot_adjustment:${entry.ot_adjustment}`, 'info');
            return entry;
        }
    ...
    async function saveAndSync(entry, skipSync = false) {
        ...
        window.addDebugLog(`Entry before strictOverrider - al_adjustment:${entry.al_adjustment}, cpl_adjustment:${entry.cpl_adjustment}, is_manual:${entry.is_manual_adjustment}`, 'info');
        ...
        window.addDebugLog(`Entry after strictOverrider - al_adjustment:${overriddenEntry.al_adjustment}, cpl_adjustment:${overriddenEntry.cpl_adjustment}, is_manual:${overriddenEntry.is_manual_adjustment}`, 'info');
    ...
    async function batchSyncToCloud() {
        ...
        try {
            ...
            for (let i = 0; i < pendingEntries.length; i++) {
                ...
                window.addDebugLog(`batchSync: RAW entry from db for ${entry.date}: al_adjustment=${entry.al_adjustment}, cpl_adjustment=${entry.cpl_adjustment}, ot_adjustment=${entry.ot_adjustment}`, 'debug');
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    window.addDebugLog(`syncFromCloud: Processed cloud entry ${entry.date} has adjustments: AL=${entry.al_adjustment}, CPL=${entry.cpl_adjustment}, OT=${entry.ot_adjustment}`, 'info');
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        sortedEntries.forEach(e => {
            window.addDebugLog(`FIFO entry ${e.date}: al_adjustment=${e.al_adjustment}, cpl_adjustment=${e.cpl_adjustment}, ot_adjustment=${e.ot_adjustment}`, 'debug');
        });
        ...
        // Track OT adjustments separately
        ...
        for (const entry of sortedEntries) {
            ...
            if (entry.al_accrued && entry.al_accrued > 0) {
                ...
                window.addDebugLog(`FIFO: Added AL accrual packet: ${entry.al_accrued} on ${entry.date}`, 'debug');
            ...
            if (alAdjustment !== 0) {
                ...
                alPackets.push(packet);
                window.addDebugLog(`FIFO: Added AL adjustment packet: ${alAdjustment} on ${entry.date}`, 'debug');
            ...
            if (entry.cpl_earned && entry.cpl_earned > 0 && entry.cpl_expiry_date) {
                ...
                window.addDebugLog(`FIFO: Added CPL earned packet: ${entry.cpl_earned} on ${entry.date}`, 'debug');
            }
            ...
            if (cplAdjustment !== 0) {
                ...
                window.addDebugLog(`FIFO: Added CPL adjustment packet: ${cplAdjustment} on ${entry.date}`, 'debug');
            }
            ...
            if (entry.al_used && entry.al_used > 0) {
                ...
                window.addDebugLog(`FIFO: Added AL usage: ${entry.al_used} on ${entry.date}`, 'debug');
            }
            ...
            if (entry.cpl_used && entry.cpl_used > 0) {
                ...
                window.addDebugLog(`FIFO: Added CPL usage: ${entry.cpl_used} on ${entry.date}`, 'debug');
            }
        }
        ...
        for (const packet of cplPacketsCopy) {
            if (packet.amount > 0 && packet.expiryDate > targetDate) {
                ...
                window.addDebugLog(`FIFO: Remaining CPL packet from ${packet.date}: ${packet.amount}`, 'debug');
            }
        }
        
        window.addDebugLog(`FIFO Matchmaker complete - AL: ${alBalance.toFixed(2)}, CPL: ${cplBalance.toFixed(2)}, OT Adjustment Total: ${totalOTAdjustment}`, 'success');
    ...
    async function loadBalances() {
        ...
        entries.forEach(e => {
            window.addDebugLog(`loadBalances: Entry from db ${e.date}: al_adjustment=${e.al_adjustment}, cpl_adjustment=${e.cpl_adjustment}, ot_adjustment=${e.ot_adjustment}`, 'debug');
        });
        ...
        for (const entry of sortedEntries) {
            ...
            // Handle SL/CL adjustments (these are separate from usage)
            // Note: The adjustment modal currently doesn't have SL/CL fields,
            // but we'll add this logic for future compatibility
            if (entry.sl_adjustment && entry.sl_adjustment !== 0) {
                totalSLAdjustment += parseFloat(entry.sl_adjustment);
                window.addDebugLog(`loadBalances: Found SL adjustment: ${entry.sl_adjustment} on ${entry.date}`, 'debug');
            }
            if (entry.cl_adjustment && entry.cl_adjustment !== 0) {
                totalCLAdjustment += parseFloat(entry.cl_adjustment);
                window.addDebugLog(`loadBalances: Found CL adjustment: ${entry.cl_adjustment} on ${entry.date}`, 'debug');
            }
        ...
        // Add OT adjustments to totals
        if (fifoResult.otAdjustmentTotal !== 0) {
            totalOT += fifoResult.otAdjustmentTotal;
            // For monthly breakdown, we'd need to track OT adjustments by date
            // For now, we'll add them to current month if they're from this year
            // This is a simplification - you might want more precise date-based tracking
            otThisMonth += fifoResult.otAdjustmentTotal;
            window.addDebugLog(`loadBalances: Added OT adjustment total ${fifoResult.otAdjustmentTotal} to OT totals`, 'info');
        }
        ...
        window.addDebugLog(`Balances - AL: ${fifoResult.alBalance.toFixed(2)}, SL: ${slBalance.toFixed(2)}, CL: ${clBalance.toFixed(2)}, CPL: ${fifoResult.cplBalance.toFixed(2)}, OT: ${totalOT.toFixed(1)} (with adjustments)`, 'success');
    }
    ...
    async function saveBalanceAdjustment() {
        ...
        // Add SL and CL adjustment fields if they don't exist
        ...
        window.addDebugLog(`STEP 6: Current adjustment values before update - AL: ${entry.al_adjustment}, CPL: ${entry.cpl_adjustment}, OT: ${entry.ot_adjustment}, SL: ${entry.sl_adjustment}, CL: ${entry.cl_adjustment}`, 'info');
    ...
    async function loadAdjustments() {
        ...
        adjustments.slice(0, 20).forEach(adj => {
            // Ensure adjustment values are numbers and default to 0 if undefined/null
