...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(setupCollapsibleSections, 1000);
    ...
    async function loadExpiryInfo() {
        ...
        // Sort by expiry date (soonest first) for FIFO display
        alExpiring.sort((a, b) => a.daysUntil - b.daysUntil);
        
        // CPL Expiry (FIFO order)
        const cplExpiring = [];
    ...
    function updateExpiryUI(alExpiring, cplExpiring) {
        ...
        if (cplExpiryDiv) {
            if (cplExpiring.length === 0) {
                ...
                let html = '<h4>CPL Expiring (FIFO Order)</h4>';
    ...
    async function loadBalances() {
        ...
        for (const al of alEarned) {
            ...
            if (usedAL <= 0) {
                remainingAL += al.amount;
            } else if (usedAL >= al.amount) {
                usedAL -= al.amount;
            } else {
                remainingAL += (al.amount - usedAL);
                usedAL = 0;
            }
        }
        
        alBalance = remainingAL;
    ...
    async function saveBalanceAdjustment() {
        ...
        if (sl !== 0) {
            // For SL, we'll use adjustment field
    ...
    window.addDebugLog('app.js: Loading complete - FIFO AL implemented', 'success');
})();
