...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Setup export CSV dropdown listener
        const exportCsvBtn = document.getElementById('exportCsvBtn');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', function() {
                const year = document.getElementById('exportYear').value;
                if (year) {
                    exportAttendanceToCSV(year);
                } else {
...
window.addDebugLog('GENERAL CHECK: Line 170 reached', 'info');
    ...
    // ==================== EXPORT CSV FUNCTION ====================
    async function exportAttendanceToCSV(year) {
        ...
        try {
            ...
            if (data.success && data.csv) {
                ...
                link.href = url;
                link.setAttribute('download', `attendance_${year}_${appCurrentUser.email}.csv`);
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
                
                window.addDebugLog(`[Export] CSV exported successfully for year ${year}`, 'success');
