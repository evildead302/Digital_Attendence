...
(function() {
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        for (const entry of sortedEntries) {
            ...
            window.addDebugLog(`Matchmaker scanning ${entry.date}: al_adjustment is ${entry.al_adjustment}`, 'info');
            ...
            if (entry.al_adjustment && parseFloat(entry.al_adjustment) !== 0) {
                const packet = {
                    date: entry.date,
                    amount: parseFloat(entry.al_adjustment),
                    expiryDate: entry.al_expiry_date ? new Date(entry.al_expiry_date) : null,
                    type: 'adjustment',
                ...
                if (!alByYear[year]) alByYear[year] = 0;
                alByYear[year] += parseFloat(entry.al_adjustment);
            }
            ...
            if (entry.cpl_adjustment && parseFloat(entry.cpl_adjustment) !== 0) {
                cplPackets.push({
                    date: entry.date,
                    amount: parseFloat(entry.cpl_adjustment),
    ...
    async function loadAdjustments() {
        ...
        entries.forEach(e => {
            window.addDebugLog(`Processing table row for ${e.date}: Found al_adjustment=${e.al_adjustment || 0}, cpl_adjustment=${e.cpl_adjustment || 0}, ot_adjustment=${e.ot_adjustment || 0}, is_manual=${e.is_manual_adjustment}`, 'debug');
        });
        ...
        adjustments.slice(0, 20).forEach(adj => {
            ...
            const alValue = adj.al_adjustment !== undefined && adj.al_adjustment !== null ? adj.al_adjustment : 0;
            const cplValue = adj.cpl_adjustment !== undefined && adj.cpl_adjustment !== null ? adj.cpl_adjustment : 0;
            const otValue = adj.ot_adjustment !== undefined && adj.ot_adjustment !== null ? adj.ot_adjustment : 0;
            ...
            let details = [];
            if (alValue !== 0) details.push(`AL: ${alValue > 0 ? '+' : ''}${parseFloat(alValue).toFixed(2)}`);
            if (cplValue !== 0) details.push(`CPL: ${cplValue > 0 ? '+' : ''}${parseFloat(cplValue).toFixed(2)}`);
            if (otValue !== 0) details.push(`OT: ${otValue > 0 ? '+' : ''}${parseFloat(otValue).toFixed(1)}`);
