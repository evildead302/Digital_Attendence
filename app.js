...
(function() {
    ...
    async function checkIn() {
        ...
        window.addDebugLog(`Check-in recorded: ${timeStr} (local)`, 'success');
    ...
    async function checkOut() {
        ...
        window.addDebugLog(`Check-out recorded: ${timeStr} (local)`, 'success');
    ...
    async function loadTodayEntry() {
        ...
        try {
            const today = new Date().toISOString().split('T')[0];
            window.addDebugLog(`Loading today's entry for date: ${today}`, 'info');
            
            const entries = await window.dbAPI.getAllEntriesForUser(appCurrentUser.id);
            const todayEntry = entries.find(e => e.date === today);
            
            if (todayEntry) {
                window.addDebugLog('Found today entry', 'info');
    ...
    async function syncToCloud() {
        ...
        try {
            ...
            window.addDebugLog(`Sending ${cleanEntries.length} entries to cloud`, 'info');
            ...
            if (data.success) {
                ...
                if (data.syncedIds && data.syncedIds.length > 0) {
                    ...
                    window.addDebugLog(`Marked ${data.syncedIds.length} entries as synced`, 'success');
                }
    ...
    async function applyTemplateToRange() {
        ...
        window.addDebugLog(`Applying template from ${from} to ${to}`, 'info');
