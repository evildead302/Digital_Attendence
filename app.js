// ==================== APP.JS - REFACTORED WITH DATA CONSISTENCY ====================
(function() {
    ...
    // ==================== STRICT OVERRIDER WITH ADJUSTMENT SHIELD ====================
    async function strictOverrider(entry) {
        ...
        if (entry.is_manual_adjustment === true) {
            window.addDebugLog(`🔒 ADJUSTMENT SHIELD ACTIVE - returning entry untouched`, 'success');
        ...
        // finalOT = Math.floor(Math.min(Math.max(rawOT, 0), updatedEntry.ot_cap_rule || 0))
    ...
    async function fetchFromCloud(date) {
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.entry) {
                    ...
                    // Ensure all fields exist with proper defaults and parseFloat to prevent NaN
                    cloudEntry.al_adjustment = parseFloat(cloudEntry.al_adjustment) || 0;
                    cloudEntry.cpl_adjustment = parseFloat(cloudEntry.cpl_adjustment) || 0;
                    cloudEntry.ot_adjustment = parseFloat(cloudEntry.ot_adjustment) || 0;
                    cloudEntry.al_accrued = parseFloat(cloudEntry.al_accrued) || 0;
                    cloudEntry.al_used = parseFloat(cloudEntry.al_used) || 0;
                    cloudEntry.sl_used = parseFloat(cloudEntry.sl_used) || 0;
                    cloudEntry.cl_used = parseFloat(cloudEntry.cl_used) || 0;
                    cloudEntry.cpl_used = parseFloat(cloudEntry.cpl_used) || 0;
                    
                    window.addDebugLog(`Cloud entry loaded with adjustments: AL=${cloudEntry.al_adjustment}, CPL=${cloudEntry.cpl_adjustment}, OT=${cloudEntry.ot_adjustment}`, 'success');
        ...
        window.addDebugLog(`Created new entry with adjustment fields initialized to 0`, 'info');
    ...
    async function saveAndSync(entry, skipSync = false) {
        ...
        if (window.dbAPI) {
            await window.dbAPI.saveEntry(overriddenEntry);
            window.addDebugLog(`Saved entry for ${entry.date} to local DB`, 'success');
        }
        
        // Only trigger sync if not skipped and online
        if (!skipSync && navigator.onLine) {
            window.addDebugLog('Scheduling sync', 'info');
        ...
        try {
            ...
            for (let i = 0; i < pendingEntries.length; i++) {
                ...
                const cleanEntry = {
                    ...
                    al_accrued: parseFloat(entry.al_accrued) || 0,
                    al_adjustment: parseFloat(entry.al_adjustment) || 0,  // CRITICAL: Use exact property name
                    al_expiry_date: entry.al_expiry_date || null,
                    cpl_adjustment: parseFloat(entry.cpl_adjustment) || 0,  // CRITICAL: Use exact property name
                    cpl_expiry_date: entry.cpl_expiry_date || null,
                    ot_adjustment: parseFloat(entry.ot_adjustment) || 0,    // CRITICAL: Use exact property name
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    // Convert string numbers using parseFloat to prevent NaN
                    ...
                    entry.al_accrued = parseFloat(entry.al_accrued) || 0;
                    
                    // CRITICAL: Use exact property names with parseFloat
                    entry.al_adjustment = parseFloat(entry.al_adjustment) || 0;
                    entry.cpl_adjustment = parseFloat(entry.cpl_adjustment) || 0;
                    entry.ot_adjustment = parseFloat(entry.ot_adjustment) || 0;
                    
                    entry.is_off_day = entry.is_off_day === true || entry.is_off_day === 'true' || entry.is_off_day === 1;
                    entry.is_holiday = entry.is_holiday === true || entry.is_holiday === 'true' || entry.is_holiday === 1;
                    entry.is_manual_adjustment = entry.is_manual_adjustment === true || entry.is_manual_adjustment === 'true' || entry.is_manual_adjustment === 1;
    ...
    function calculateFIFOBalance(entries, targetDate = new Date()) {
        ...
        for (const entry of sortedEntries) {
            ...
            if (entry.al_accrued && entry.al_accrued > 0) {
                const packet = {
                    date: entry.date,
                    amount: parseFloat(entry.al_accrued) || 0,
                ...
                alByYear[year] += parseFloat(entry.al_accrued) || 0;
            }
            
            // AL Packets (adjustments) - Use exact property name al_adjustment
            if (entry.al_adjustment && parseFloat(entry.al_adjustment) !== 0) {
                const amount = parseFloat(entry.al_adjustment) || 0;
                const packet = {
                    date: entry.date,
                    amount: amount,
                    expiryDate: entry.al_expiry_date ? new Date(entry.al_expiry_date) : null,
                ...
                alPackets.push(packet);
                window.addDebugLog(`Added AL adjustment packet: ${amount} on ${entry.date}`, 'info');
                
                if (!alByYear[year]) alByYear[year] = 0;
                alByYear[year] += amount;
            }
            ...
            if (entry.cpl_earned && entry.cpl_earned > 0 && entry.cpl_expiry_date) {
                cplPackets.push({
                    date: entry.date,
                    amount: parseFloat(entry.cpl_earned) || 0,
                    expiryDate: new Date(entry.cpl_expiry_date),
                    entryId: entry.date
                });
            }
            
            // CPL Adjustment Packets - Use exact property name cpl_adjustment
            if (entry.cpl_adjustment && parseFloat(entry.cpl_adjustment) !== 0) {
                const amount = parseFloat(entry.cpl_adjustment) || 0;
                cplPackets.push({
                    date: entry.date,
                    amount: amount,
                ...
                window.addDebugLog(`Added CPL adjustment packet: ${amount} on ${entry.date}`, 'info');
            }
            ...
            if (entry.al_used && entry.al_used > 0) {
                alUsage.push({
                    date: entry.date,
                    amount: parseFloat(entry.al_used) || 0,
                    entryId: entry.date
                });
            }
            ...
            if (entry.cpl_used && entry.cpl_used > 0) {
                cplUsage.push({
                    date: entry.date,
                    amount: parseFloat(entry.cpl_used) || 0,
                    entryId: entry.date
                });
            }
        }
    ...
    async function loadBalances() {
        ...
        for (const entry of sortedEntries) {
            ...
            if (entry.sl_used) {
                slBalance -= parseFloat(entry.sl_used) || 0;
                totalLeave += parseFloat(entry.sl_used) || 0;
            }
            if (entry.cl_used) {
                clBalance -= parseFloat(entry.cl_used) || 0;
                totalLeave += parseFloat(entry.cl_used) || 0;
            }
            if (entry.al_used) totalLeave += parseFloat(entry.al_used) || 0;
            if (entry.cpl_used) totalLeave += parseFloat(entry.cpl_used) || 0;
    ...
    async function markLeave(type) {
        ...
        entry[`${type}_used`] = (parseFloat(entry[`${type}_used`]) || 0) + 1;
        entry.check_in = null;
        entry.check_out = null;
    ...
    async function saveBalanceAdjustment() {
        ...
        // Store adjustments in separate columns - use parseFloat to prevent NaN
        if (al !== 0) {
            const oldValue = parseFloat(entry.al_adjustment) || 0;
            entry.al_adjustment = oldValue + al;
        ...
        if (cpl !== 0) {
            const oldValue = parseFloat(entry.cpl_adjustment) || 0;
            entry.cpl_adjustment = oldValue + cpl;
        ...
        if (ot !== 0) {
            const oldValue = parseFloat(entry.ot_adjustment) || 0;
            entry.ot_adjustment = oldValue + ot;
    ...
    async function loadAdjustments() {
        ...
        const adjustments = entries.filter(e => 
            ...
            (e.al_adjustment && parseFloat(e.al_adjustment) !== 0) ||
            (e.cpl_adjustment && parseFloat(e.cpl_adjustment) !== 0) ||
            (e.ot_adjustment && parseFloat(e.ot_adjustment) !== 0)
        );
        ...
        adjustments.slice(0, 20).forEach(adj => {
            ...
            let details = [];
            if (adj.al_adjustment && parseFloat(adj.al_adjustment) !== 0) details.push(`AL: ${parseFloat(adj.al_adjustment) > 0 ? '+' : ''}${parseFloat(adj.al_adjustment).toFixed(2)}`);
            if (adj.cpl_adjustment && parseFloat(adj.cpl_adjustment) !== 0) details.push(`CPL: ${parseFloat(adj.cpl_adjustment) > 0 ? '+' : ''}${parseFloat(adj.cpl_adjustment).toFixed(2)}`);
            if (adj.ot_adjustment && parseFloat(adj.ot_adjustment) !== 0) details.push(`OT: ${parseFloat(adj.ot_adjustment) > 0 ? '+' : ''}${parseFloat(adj.ot_adjustment).toFixed(1)}`);
    ...
    async function saveInitialBalances() {
        ...
        // Store as adjustments - use parseFloat to prevent NaN
        if (al !== 0) {
            entry.al_adjustment = (parseFloat(entry.al_adjustment) || 0) + al;
        ...
        if (cpl !== 0) {
            entry.cpl_adjustment = (parseFloat(entry.cpl_adjustment) || 0) + cpl;
        ...
        if (ot !== 0) {
            entry.ot_adjustment = (parseFloat(entry.ot_adjustment) || 0) + ot;
    ...
    window.addDebugLog('app.js: Loading complete - REFACTORED WITH DATA CONSISTENCY', 'success');
})();
