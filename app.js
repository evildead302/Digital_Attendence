// ==================== APP.JS - COMPLETE WITH BULK DATE RANGE ====================
(function() {
    ...
    async function recalculateEntry(entry) {
        ...
        if ((isHoliday || updatedEntry.is_holiday) && updatedEntry.check_in && updatedEntry.check_out) {
            ...
            window.addDebugLog(`CPL earned: ${cplGrant} (expires: ${updatedEntry.cpl_expiry_date})`, 'success');
        } else {
            updatedEntry.cpl_earned = 0;
            updatedEntry.cpl_expiry_date = null;
        }
    ...
    async function recalculateAllMonthEnds() {
        ...
        // Also check previous year's December for carry forward
    ...
    function updateExpiryUI(alExpiring, cplExpiring) {
        ...
        if (alExpiryDiv) {
            if (alExpiring.length === 0) {
                ...
                // Show ALL years with expiring AL
                let html = '<h4>AL Expiry by Year</h4>';
                alExpiring.forEach(item => {
                    const carryForward = Math.min(item.amount, 22);
                    const expires = item.amount > 22 ? (item.amount - 22).toFixed(2) : 0;
                    html += `
                        <div class="expiry-item">
                            <div><strong>Year ${item.year}</strong></div>
                            <div>Total: ${item.amount.toFixed(2)} days</div>
                            <div>Carry forward: ${carryForward.toFixed(2)} days</div>
                            ${expires > 0 ? `<div style="color: #f44336;">Will expire: ${expires} days</div>` : ''}
    ...
    async function loadBalances() {
        ...
        // Also add any AL that hasn't expired but is from current year (full amount)
        const currentYearExpiry = currentYear + 1;
        if (alByExpiryYear[currentYearExpiry]) {
            totalAlBalance += alByExpiryYear[currentYearExpiry];
        }
    ...
    async function loadAdjustments() {
        ...
        // Show ALL adjustments (any entry with al_accrued, cpl_earned, or adjustment_note)
        ...
        list.innerHTML = '<h4>All Adjustments</h4>';
        ...
        adjustments.slice(0, 20).forEach(adj => {
            ...
            // Determine type
            let type = 'Manual';
            if (adj.adjustment_note === 'Initial balance setup') type = 'Initial';
            else if (adj.is_holiday && adj.cpl_earned > 0) type = 'Holiday';
            else if (adj.al_accrued === 1.833) type = 'Monthly Accrual';
            
            item.innerHTML = `
                <div class="adjustment-date">${adj.date} [${type}]</div>
    ...
    async function saveSingleDateOverride() {
        ...
        if (type === 'work') {
            ...
            // Set CPL rule (will be applied only if worked)
            if (cplInput !== '') {
    ...
    async function applyTemplateToRange() {
        ...
        try {
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                ...
                // Recalculate and save (this will set cpl_earned only if worked)
                await recalculateAndSaveEntry(entry);
    ...
    function displayHistory(entries) {
        ...
        entries.slice(0, 50).forEach(e => {
            ...
            if (e.is_off_day) {
                ...
                // Parse local time (already in local format)
    ...
    window.addDebugLog('app.js: Loading complete - Bulk entry added', 'success');
})();
