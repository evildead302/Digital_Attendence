...
(function() {
    ...
    // This function should be called after successful login to enable/disable debug based on user email
    function updateDebugForLoggedInUser(email) {
        if (typeof window.updateDebugForUser === 'function') {
            window.updateDebugForUser(email);
            window.addDebugLog(`Debug system updated for user: ${email}`, 'info');
        } else {
            console.log(`updateDebugForUser not available, debug may not work for ${email}`);
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Setup export CSV button listener (updated for date range)
        const exportCsvBtn = document.getElementById('exportCsvBtn');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', function() {
                exportAttendanceToCSV(); // No parameter needed anymore
            });
        }
        
        // Setup OT toggle button listener (home screen)
        const otToggleBtn = document.getElementById('otToggleBtn');
        if (otToggleBtn) {
            otToggleBtn.addEventListener('click', function() {
                userSettings.has_ot = !userSettings.has_ot;
                // Also update the settings tab toggle switch
                const settingsOtToggle = document.getElementById('userHasOT');
                if (settingsOtToggle) settingsOtToggle.checked = userSettings.has_ot;
                updateOCToggleButtons();
                saveUserSettingsAndApply();
            });
        }
        
        // Setup CPL toggle button listener (home screen)
        const cplToggleBtn = document.getElementById('cplToggleBtn');
        if (cplToggleBtn) {
            cplToggleBtn.addEventListener('click', function() {
                userSettings.has_cpl = !userSettings.has_cpl;
                // Also update the settings tab toggle switch
                const settingsCplToggle = document.getElementById('userHasCPL');
                if (settingsCplToggle) settingsCplToggle.checked = userSettings.has_cpl;
                updateOCToggleButtons();
                saveUserSettingsAndApply();
            });
        }
        
        // Setup settings tab toggle listeners
        const settingsOtToggle = document.getElementById('userHasOT');
        const settingsCplToggle = document.getElementById('userHasCPL');
        
        if (settingsOtToggle) {
            settingsOtToggle.addEventListener('change', function() {
                userSettings.has_ot = settingsOtToggle.checked;
                updateOCToggleButtons();
            });
        }
        
        if (settingsCplToggle) {
            settingsCplToggle.addEventListener('change', function() {
                userSettings.has_cpl = settingsCplToggle.checked;
                updateOCToggleButtons();
            });
        }
    });
    ...
    async function exportAttendanceToCSV() {
        ...
        window.addDebugLog(`[Export] Exporting attendance data from ${fromDate} to ${toDate}`, 'info');
        
        try {
            const response = await fetch(`/api/account?action=export&from=${fromDate}&to=${toDate}`, {
                method: 'GET',
            ...
            // FIXED: Get response as TEXT (CSV content), not JSON
            const csvContent = await response.text();
            
            // FIXED: Check if we got valid CSV content
            if (csvContent && csvContent.length > 0 && (csvContent.includes('Date') || csvContent.includes(','))) {
                ...
                window.addDebugLog(`[Export] CSV exported successfully for date range`, 'success');
                alert(`✅ Attendance data exported successfully!`);
            } else {
                throw new Error('No valid CSV data received from server');
            }
        } catch (error) {
    ...
    // ==================== UPDATE OT/CPL TOGGLE BUTTONS ====================
    function updateOCToggleButtons() {
        const otToggleBtn = document.getElementById('otToggleBtn');
        const cplToggleBtn = document.getElementById('cplToggleBtn');
        
        if (otToggleBtn) {
            otToggleBtn.textContent = userSettings.has_ot ? '✅ OT Enabled' : '❌ OT Disabled';
            otToggleBtn.style.backgroundColor = userSettings.has_ot ? '#4caf50' : '#f44336';
        }
        
        if (cplToggleBtn) {
            cplToggleBtn.textContent = userSettings.has_cpl ? '✅ CPL Enabled' : '❌ CPL Disabled';
            cplToggleBtn.style.backgroundColor = userSettings.has_cpl ? '#4caf50' : '#f44336';
        }
        
        window.addDebugLog(`[Settings] OT: ${userSettings.has_ot}, CPL: ${userSettings.has_cpl}`, 'info');
    }
    ...
    async function strictOverrider(entry, isActiveEdit = false) {
        ...
        // ===== STEP 10: Smart sync status - only mark as pending if entry has actually changed =====
        // Compare with original entry to determine if changes were made
        if (entry && JSON.stringify(entry) !== JSON.stringify(updatedEntry)) {
            updatedEntry.sync_status = 'pending';
            window.addDebugLog(`Entry changed since last sync - marking as pending`, 'info');
        } else if (entry && entry.sync_status === 'synced') {
            updatedEntry.sync_status = 'synced';
            window.addDebugLog(`Entry unchanged - keeping sync_status as synced`, 'info');
        } else {
            updatedEntry.sync_status = 'pending';
            window.addDebugLog(`No previous entry or sync_status not set - marking as pending`, 'info');
        }
        
        window.addDebugLog(`STRICT OVERRIDER complete - Hours: ${hoursWorked.toFixed(2)}, Base: ${updatedEntry.base_hours_rule}, OT: ${updatedEntry.final_ot_hours}, CPL: ${updatedEntry.cpl_earned}, Holiday: ${updatedEntry.is_holiday}, sync_status: ${updatedEntry.sync_status}`, 'success');
    ...
    // FIXED: fetchFromCloud - NO UTC CONVERSION, treat cloud data as local datetime strings directly
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.entry) {
                    ...
                    // CRITICAL FIX: Cloud stores local datetime strings, NOT UTC
                    // Do NOT convert - keep as-is
                    // The cloud stores times like "2026-05-11 00:30:00" directly
                    // So we should NOT do any UTC to local conversion
                    if (cloudEntry.check_in && typeof cloudEntry.check_in === 'string') {
                        // Just ensure it's in the correct format (replace T with space if needed)
                        if (cloudEntry.check_in.includes('T')) {
                            cloudEntry.check_in = cloudEntry.check_in.replace('T', ' ');
                        }
                        // Remove any timezone suffix if present
                        cloudEntry.check_in = cloudEntry.check_in.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`fetchFromCloud: Keeping check_in as stored: ${cloudEntry.check_in}`, 'info');
                    }
                    
                    if (cloudEntry.check_out && typeof cloudEntry.check_out === 'string') {
                        if (cloudEntry.check_out.includes('T')) {
                            cloudEntry.check_out = cloudEntry.check_out.replace('T', ' ');
                        }
                        cloudEntry.check_out = cloudEntry.check_out.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`fetchFromCloud: Keeping check_out as stored: ${cloudEntry.check_out}`, 'info');
                    }
    ...
    // ==================== SINGLE ENTRY SYNC (FIXED - NO UTC CONVERSION) ====================
    async function syncSingleEntryToCloud(entry) {
        if (!appAuthToken || !appCurrentUser) return false;
        if (!navigator.onLine) {
        ...
        try {
            // Clean entry for sync - KEEP AS LOCAL DATETIME STRINGS, NO UTC CONVERSION
            let checkInLocal = null;
            let checkOutLocal = null;
            
            if (entry.check_in) {
                let checkInStr = entry.check_in;
                if (checkInStr.includes('T')) checkInStr = checkInStr.replace('T', ' ');
                checkInLocal = checkInStr; // Keep as local datetime string
                window.addDebugLog(`syncSingleEntryToCloud: Keeping check_in as local: ${checkInLocal}`, 'debug');
            }
            
            if (entry.check_out) {
                let checkOutStr = entry.check_out;
                if (checkOutStr.includes('T')) checkOutStr = checkOutStr.replace('T', ' ');
                checkOutLocal = checkOutStr; // Keep as local datetime string
                window.addDebugLog(`syncSingleEntryToCloud: Keeping check_out as local: ${checkOutLocal}`, 'debug');
            }
            ...
            // FIXED: Use the correct endpoint - /api/account?action=sync-single
            const response = await fetch('/api/account?action=sync-single', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${appAuthToken}`,
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ entry: cleanEntry })
            });
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    window.addDebugLog(`Successfully synced single entry ${entry.date}`, 'success');
                    return true;
                }
            }
            window.addDebugLog(`Failed to sync ${entry.date}: ${response.status}`, 'error');
            return false;
        } catch (error) {
            window.addDebugLog(`Error syncing single entry: ${error.message}`, 'error');
            return false;
        }
    }

    // ==================== SAVE WITH STRICT OVERRIDE (UPDATED) ====================
    async function saveAndSync(entry, skipSync = false, isActiveEdit = true) {
        ...
        // ONLY sync THIS entry, not all pending entries
        if (!skipSync && navigator.onLine) {
            window.addDebugLog(`saveAndSync: Syncing JUST entry ${entry.date} to cloud`, 'info');
            await syncSingleEntryToCloud(overriddenEntry);
        }
    ...
    // ==================== BATCH SYNC (FIXED - NO UTC CONVERSION) ====================
        ...
        try {
            ...
            for (let i = 0; i < pendingEntries.length; i++) {
                ...
                if (entry.check_in) {
                    ...
                    if (checkInStr.includes('T')) checkInStr = checkInStr.replace('T', ' ');
                    checkInLocal = checkInStr; // Keep as local datetime string
                    window.addDebugLog(`batchSync: Keeping check_in as local: "${checkInLocal}"`, 'debug');
                }
                ...
                if (entry.check_out) {
                    ...
                    if (checkOutStr.includes('T')) checkOutStr = checkOutStr.replace('T', ' ');
                    checkOutLocal = checkOutStr; // Keep as local datetime string
                    window.addDebugLog(`batchSync: Keeping check_out as local: "${checkOutLocal}"`, 'debug');
                }
    ...
    // FIXED: syncFromCloud - NO UTC CONVERSION, treat cloud data as local datetime strings directly
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    // Do NOT convert - keep as-is
                    // The cloud stores times like "2026-05-11 00:30:00" directly
                    if (entry.check_in && typeof entry.check_in === 'string') {
                        // Just ensure it's in the correct format (replace T with space if needed)
                        if (entry.check_in.includes('T')) {
                            entry.check_in = entry.check_in.replace('T', ' ');
                        }
                        // Remove any timezone suffix if present
                        entry.check_in = entry.check_in.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`syncFromCloud: Keeping check_in as stored: ${entry.check_in}`, 'info');
                    }
                    
                    if (entry.check_out && typeof entry.check_out === 'string') {
                        if (entry.check_out.includes('T')) {
                            entry.check_out = entry.check_out.replace('T', ' ');
                        }
                        entry.check_out = entry.check_out.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`syncFromCloud: Keeping check_out as stored: ${entry.check_out}`, 'info');
                    }
        ...
        try {
            ...
            if (data.success && data.entries) {
                ...
                for (const entry of data.entries) {
                    ...
                    // FIXED: RECALC - treat cloud data as local datetime strings, no conversion needed
                    if (entry.check_in && typeof entry.check_in === 'string') {
                        if (entry.check_in.includes('T')) {
                            entry.check_in = entry.check_in.replace('T', ' ');
                        }
                        entry.check_in = entry.check_in.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`recalculateAll: Keeping check_in as stored: ${entry.check_in}`, 'info');
                    }
                    ...
                    if (entry.check_out && typeof entry.check_out === 'string') {
                        ...
                        entry.check_out = entry.check_out.replace(/Z$/, '').replace(/[+-]\d{2}:\d{2}$/, '');
                        window.addDebugLog(`recalculateAll: Keeping check_out as stored: ${entry.check_out}`, 'info');
                    }
    ...
    function updateSettingsUI() {
        // Update settings tab toggle switches
        const otToggleSwitch = document.getElementById('userHasOT');
        const cplToggleSwitch = document.getElementById('userHasCPL');
        ...
        if (otToggleSwitch) otToggleSwitch.checked = userSettings.has_ot;
        if (cplToggleSwitch) cplToggleSwitch.checked = userSettings.has_cpl;
        ...
        // Also update the home screen toggle buttons
        updateOCToggleButtons();
    }
    ...
    async function saveUserSettingsAndApply() {
        ...
        // READ VALUES FROM SETTINGS TAB TOGGLE SWITCHES
        const otToggleSwitch = document.getElementById('userHasOT');
        const cplToggleSwitch = document.getElementById('userHasCPL');
        const annualLimit = document.getElementById('userLimitAnnual');
        const casualLimit = document.getElementById('userLimitCasual');
        ...
        // Get values from settings tab controls
        const newHasOT = otToggleSwitch ? otToggleSwitch.checked : userSettings.has_ot;
        const newHasCPL = cplToggleSwitch ? cplToggleSwitch.checked : userSettings.has_cpl;
        const newLimitAnnual = annualLimit ? parseInt(annualLimit.value) || 0 : userSettings.limit_annual;
        const newLimitCasual = casualLimit ? parseInt(casualLimit.value) || 0 : userSettings.limit_casual;
        const newLimitSick = sickLimit ? parseInt(sickLimit.value) || 0 : userSettings.limit_sick;
        
        // Update userSettings object
        userSettings.has_ot = newHasOT;
        userSettings.has_cpl = newHasCPL;
        userSettings.limit_annual = newLimitAnnual;
        userSettings.limit_casual = newLimitCasual;
        userSettings.limit_sick = newLimitSick;
        ...
        if (saved) {
            // Update the toggle buttons in home screen to match
            updateOCToggleButtons();
    ...
    // ==================== ENHANCED PUSH SUBSCRIPTION SYNC WITH RETRY LOGIC & TIMEOUT & OVERRIDE ====================
    ...
    async function syncPushSubscription(retryCount = 0) {
        ...
        try {
            ...
            // ============================================================
            // GET VAPID PUBLIC KEY FROM SERVER API FIRST
            // ============================================================
            let vapidPublicKey = null;
            
            // FIRST: Try to fetch from server API (most reliable)
            window.addDebugLog('[Push] Fetching VAPID key from server API...', 'info');
            try {
                const vapidResponse = await fetch('/api/notifications?action=get-vapid-key');
                window.addDebugLog(`[Push] VAPID fetch response status: ${vapidResponse.status}`, 'info');
                
                if (vapidResponse.ok) {
                    const vapidData = await vapidResponse.json();
                    window.addDebugLog(`[Push] VAPID fetch response: ${JSON.stringify(vapidData)}`, 'debug');
                    
                    if (vapidData.success && vapidData.vapidPublicKey) {
                        vapidPublicKey = vapidData.vapidPublicKey;
                        window.addDebugLog(`[Push] ✅ VAPID key fetched from server successfully! Source: ${vapidData.source || 'server'}`, 'success');
                        window.addDebugLog(`[Push] VAPID key (first 30 chars): ${vapidPublicKey.substring(0, 30)}...`, 'debug');
                    } else {
                        window.addDebugLog(`[Push] Server returned failure: ${vapidData.error || vapidData.message || 'Unknown error'}`, 'warning');
                    }
                } else {
                    window.addDebugLog(`[Push] Server returned ${vapidResponse.status} for VAPID key request`, 'warning');
                }
            } catch (vapidError) {
                window.addDebugLog(`[Push] Failed to fetch VAPID key from server: ${vapidError.message}`, 'warning');
            }
            
            // SECOND: Try meta tag as fallback
            if (!vapidPublicKey) {
                ...
                if (vapidMeta && vapidMeta.content && vapidMeta.content !== '%VAPID_PUBLIC_KEY%' && vapidMeta.content !== '') {
                    ...
                    window.addDebugLog('[Push] VAPID key found in meta tag (fallback)', 'success');
                }
            }
            
            // THIRD: Try window variable as fallback
            if (!vapidPublicKey && window.VAPID_PUBLIC_KEY && window.VAPID_PUBLIC_KEY !== '%VAPID_PUBLIC_KEY%' && window.VAPID_PUBLIC_KEY !== '') {
                ...
                window.addDebugLog('[Push] VAPID key found in window variable (fallback)', 'success');
            }
            
            // If no existing subscription and we have a VAPID key, create one
            if (!subscription && vapidPublicKey) {
                window.addDebugLog('[Push] No existing subscription, creating new one with VAPID key', 'info');
                try {
                    ...
                    window.addDebugLog('[Push] Calling pushManager.subscribe...', 'info');
                    const rawSubscription = await registration.pushManager.subscribe(subscribeOptions);
                    
                    // ============================================================
                    // KEY EXTRACTION - Mobile Chrome compatible
                    // ============================================================
                    let p256dhKey = null;
                    let authKey = null;
                    let extractionMethod = 'none';
                    
                    // Method 1: Try getKey() (works on mobile Chrome)
                    if (rawSubscription.getKey) {
                        window.addDebugLog('[Push] Trying getKey() method...', 'debug');
                        ...
                        if (p256dhBuffer && authBuffer) {
                            ...
                            for (let i = 0; i < p256dhArray.length; i++) {
                                p256dhBinary += String.fromCharCode(p256dhArray[i]);
                            }
                            for (let i = 0; i < authArray.length; i++) {
                                authBinary += String.fromCharCode(authArray[i]);
                            }
                            
                            p256dhKey = btoa(p256dhBinary);
                            authKey = btoa(authBinary);
                            extractionMethod = 'getKey()';
                            window.addDebugLog('[Push] Keys extracted via getKey()', 'success');
                        }
                    }
                    
                    // Method 2: Try toJSON() as fallback
                    if (!p256dhKey && rawSubscription.toJSON) {
                        window.addDebugLog('[Push] Trying toJSON() method...', 'debug');
                        const json = rawSubscription.toJSON();
                        if (json && json.keys && json.keys.p256dh && json.keys.auth) {
                            p256dhKey = json.keys.p256dh;
                            authKey = json.keys.auth;
                            extractionMethod = 'toJSON()';
                            window.addDebugLog('[Push] Keys extracted via toJSON()', 'success');
                        }
                    }
                    
                    // Method 3: Try direct keys (desktop Chrome fallback)
                    if (!p256dhKey && rawSubscription.keys && rawSubscription.keys.p256dh) {
                        p256dhKey = rawSubscription.keys.p256dh;
                        authKey = rawSubscription.keys.auth;
                        extractionMethod = 'direct keys';
                        window.addDebugLog('[Push] Keys extracted via direct keys', 'success');
                    }
                    
                    if (!p256dhKey || !authKey) {
                        window.addDebugLog('[Push] ❌ Failed to extract keys using any method!', 'error');
                        localStorage.setItem('push_subscription_status', 'extraction_failed');
                        return false;
                    }
                    
                    window.addDebugLog(`[Push] Keys extracted using: ${extractionMethod}`, 'success');
                    window.addDebugLog(`[Push] p256dh (first 30): ${p256dhKey.substring(0, 30)}...`, 'debug');
                    window.addDebugLog(`[Push] auth (first 30): ${authKey.substring(0, 30)}...`, 'debug');
                    
                    // Create clean subscription object for server
                    subscription = {
                    ...
                    window.addDebugLog('[Push] ✅ New subscription created and validated!', 'success');
                    window.addDebugLog(`[Push] Endpoint: ${subscription.endpoint.substring(0, 60)}...`, 'debug');
                    
                } catch (subError) {
                    window.addDebugLog(`[Push] Subscription creation failed: ${subError.message}`, 'error');
                    ...
                    if (subError.message.includes('userVisibleOnly')) {
                        window.addDebugLog('[Push] Browser may not support userVisibleOnly option', 'error');
                    }
                ...
                window.addDebugLog('[Push] Please ensure VAPID_PUBLIC_KEY is set in Vercel environment variables', 'error');
            ...
            if (!subscription || !subscription.endpoint || !subscription.keys || !subscription.keys.p256dh || !subscription.keys.auth) {
                window.addDebugLog('[Push] Invalid subscription - missing required fields before sending to server', 'error');
                window.addDebugLog(`[Push] Subscription state: endpoint=${!!subscription?.endpoint}, keys=${!!subscription?.keys}, p256dh=${!!subscription?.keys?.p256dh}, auth=${!!subscription?.keys?.auth}`, 'error');
                localStorage.setItem('push_subscription_status', 'invalid_subscription');
                return false;
            }
            
            // If we have a subscription, send it to the server (this will override existing subscription for this user)
            if (subscription) {
                window.addDebugLog('[Push] Sending subscription to server (will override existing)...', 'info');
                ...
                try {
                    const response = await fetch('/api/notifications?action=subscribe&override=true', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${appAuthToken}`
                    ...
                    if (response.ok && responseData.success) {
                        window.addDebugLog('[Push] ✅ Subscription synced with server successfully (existing overridden)!', 'success');
    ...
    async function checkAuth() {
        ...
        if (token && userStr) {
            try {
                ...
                // Update debug system based on logged in user email
                updateDebugForLoggedInUser(appCurrentUser.email);
    ...
    // FIXED: loadTodayEntry with AM/PM display
    ...
    function switchTab(tabName) {
        ...
        if (tabName === 'settings') {
            ...
            if (exportFromDate && exportToDate) {
                ...
                exportToDate.value = today.toISOString().split('T')[0];
                exportFromDate.value = oneYearAgo.toISOString().split('T')[0];
            }
            setTimeout(setupCollapsibleSections, 500);
        }
    }

    // ==================== EXPORT CSV (UPDATED - Already done above) ====================
    // The exportAttendanceToCSV function was already updated at the top
