...
(function() {
    ...
    async function recalculateCheckoutAlarm(date) {
        ...
        // Get current check-in time (stored as local time in ledger)
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    window.addDebugLog('[Alarm] Settings loaded (UTC to Local conversion applied):', currentAlarmSettings);
                }
            }
        } catch (error) {
    ...
    async function saveAlarmSettings() {
        ...
        window.addDebugLog(`[Alarm] Saving: enabled=${isEnabled}, local_checkin=${checkinTime}, local_checkout=${localCheckoutTime}, utc_checkout=${utcCheckoutTime}, offset=${tzOffset}`, 'info');
    ...
    async function updateTargetTimeDisplay() {
        ...
        // Path B: Get actual base hours and OT from ledger, don't default to 0
