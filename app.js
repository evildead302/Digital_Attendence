...
(function() {
    ...
    async function recalculateEntry(entry) {
        ...
        // Handle CPL - ONLY if worked on holiday AND hours worked >= base
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            try {
                ...
                if (type === 'work') {
                    ...
                    // Only set CPL if worked and hours >= base
                    if (entry.check_in && entry.check_out) {
                        ...
                        // Will be recalculated properly in recalculateEntry
                    }
    ...
    async function saveManualEntry() {
        ...
        if (type === 'work') {
            ...
            // Only set CPL if worked (will be properly calculated in recalculateEntry)
            if (entry.check_in && entry.check_out) {
                ...
                if (dayName === 'sunday') {
                    const sundayWeek = getSundayWeekNumber(entryDate);
                    entry.cpl_grant_rule = (sundayWeek % 2 === 1) ? 1.0 : 0.5;
                } else {
    ...
    async function createOrUpdateMonthEndEntry(dateStr, year, month) {
        ...
        window.addDebugLog(`Creating/updating month end entry for ${dateStr}`, 'info');
        ...
        if (entry) {
            ...
            await window.dbAPI.saveEntry(entry);
            window.addDebugLog(`✅ Updated month end entry for ${dateStr} (expires: ${expiryDate})`, 'success');
        } else {
            // Create new entry
            const accrualEntry = {
                date: dateStr,
            ...
            await window.dbAPI.saveEntry(accrualEntry);
            window.addDebugLog(`✅ Created month end entry for ${dateStr} (expires: ${expiryDate})`, 'success');
        }
    }
    ...
    async function recalculateAllMonthEnds() {
        ...
        for (let month = 0; month <= currentMonth; month++) {
            ...
            // Always create/update month end entry
