...
(function() {
    ...
    async function recalculateEntry(entry) {
        ...
        // If no check-in/out, nothing to calculate - IMPORTANT FIX: No CPL without check-in/out
        if (!updatedEntry.check_in || !updatedEntry.check_out) {
            window.addDebugLog('No check-in/out times, setting CPL to 0 even for holidays', 'info');
            updatedEntry.cpl_earned = 0;
            updatedEntry.cpl_expiry_date = null;
            return updatedEntry;
        }
        ...
        // Handle CPL - ONLY if worked (check-in/out exists)
    ...
    async function saveManualEntry() {
        ...
        if (type === 'work') {
            ...
            // Only set CPL if check-in/out exists
            if (checkIn && checkOut) {
                ...
                window.addDebugLog('Holiday marked but no check-in/out - no CPL earned', 'info');
            }
    ...
    async function loadExpiryInfo() {
        ...
        // AL Expiry - Only show the soonest expiring AL
        const alExpiring = [];
        const alEntries = entries.filter(e => e.al_accrued > 0 && e.al_expiry_date);
        
        // Group AL by expiry year and calculate total per year
        ...
        // CPL Expiry - Show all expiring CPL
        const cplExpiring = [];
    ...
    function updateExpiryUI(alExpiring, cplExpiring) {
        ...
        if (alExpiryDiv) {
            if (alExpiring.length === 0) {
                ...
                // Show only the soonest expiring AL (first in array)
                const soonest = alExpiring[0];
                // Calculate how much can be carried forward (max 22 days)
                const carryForward = Math.min(soonest.amount, 22);
                const expires = soonest.amount > 22 ? soonest.amount - 22 : 0;
                
                alExpiryDiv.innerHTML = `
                    <h4>AL Expiry (Year ${soonest.year})</h4>
                    <div class="expiry-item">
                        <span>Total: ${soonest.amount.toFixed(2)} days</span>
                        <span>Expires in ${soonest.daysUntil} days</span>
                    </div>
                    <div class="expiry-item" style="background: #e3f2fd;">
                        <span>Can carry forward: ${carryForward.toFixed(2)} days</span>
                        <span>Will expire: ${expires.toFixed(2)} days</span>
                    </div>
        ...
        if (cplExpiryDiv) {
            if (cplExpiring.length === 0) {
                ...
                // Show only the soonest expiring CPL
                const soonest = cplExpiring[0];
                cplExpiryDiv.innerHTML = `
                    <h4>CPL Expiring Soon</h4>
                    <div class="expiry-item">
                        <span>${soonest.amount.toFixed(2)} days</span>
                        <span>Expires in ${soonest.daysUntil} days</span>
                    </div>
                    <p style="font-size: 11px; color: #666; margin-top: 5px;">
                        From ${soonest.date}
    ...
    async function loadBalances() {
        ...
        const alEarned = []; // Array of {date, amount, expiryDate}
        let totalALUsed = 0;
        ...
        // ===== CALCULATE AL BALANCE USING FIFO =====
    ...
    async function loadAdjustments() {
        ...
        const entries = await window.dbAPI.getAllEntriesForUser(appCurrentUser.id);
        // Only show actual adjustments (with note or non-zero adjustments), not scheduled holidays
        const adjustments = entries.filter(e => 
            (e.adjustment_note && e.adjustment_note.length > 0) ||
            (e.al_accrued && e.al_accrued > 0 && e.adjustment_note === 'Initial balance setup')
        );
    ...
    async function saveSingleDateOverride() {
        ...
        if (type === 'work') {
            ...
            // Set CPL (will only be applied if check-in/out exists)
            if (cplInput !== '') {
        ...
        try {
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                ...
                if (dayName === 'sunday') {
                    ...
                    // Note: cpl_earned will only be set if check-in/out exists during recalculation
                } else {
    ...
    async function loadHistory() {
        ...
        switch(currentHistoryFilter) {
            case 'ot':
                ...
                break;
            case 'cpl':
                entries = entries.filter(e => e.cpl_earned > 0); // Only show when actually earned
                break;
            case 'leave':
    ...
    window.addDebugLog('app.js: Loading complete - All issues fixed', 'success');
})();