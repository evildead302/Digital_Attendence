// ==================== DB.JS - INDEXEDDB WITH ALL METHODS ====================
(function() {
    ...
    async function openUserDatabase(userId) {
        ...
        return new Promise((resolve, reject) => {
            const request = indexedDB.open(dbName, 8); // Version 8 to match cloud schema
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Create entries store with all fields matching cloud schema
                if (!db.objectStoreNames.contains('entries')) {
    ...
    // Save entry with optimized write - MATCHING CLOUD SCHEMA
    async function saveEntry(entry) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                // Create entry object matching cloud schema exactly
                const completeEntry = {
                    // Core fields - match cloud column names
                    date: entry.date,
                    ...
                    // Rule fields with defaults matching cloud
                    ...
                    // Calculated fields
                    final_ot_hours: entry.final_ot_hours !== undefined && entry.final_ot_hours !== null ? entry.final_ot_hours : 0,
                    cpl_earned: entry.cpl_earned !== undefined && entry.cpl_earned !== null ? entry.cpl_earned : 0,
                    ...
                    // ADJUSTMENT FIELDS - ALL FIVE TYPES matching cloud
                    al_adjustment: entry.al_adjustment !== undefined && entry.al_adjustment !== null ? parseFloat(entry.al_adjustment) : 0,
                    sl_adjustment: entry.sl_adjustment !== undefined && entry.sl_adjustment !== null ? parseFloat(entry.sl_adjustment) : 0,
                    cl_adjustment: entry.cl_adjustment !== undefined && entry.cl_adjustment !== null ? parseFloat(entry.cl_adjustment) : 0,
                    cpl_adjustment: entry.cpl_adjustment !== undefined && entry.cpl_adjustment !== null ? parseFloat(entry.cpl_adjustment) : 0,
                    ot_adjustment: entry.ot_adjustment !== undefined && entry.ot_adjustment !== null ? parseFloat(entry.ot_adjustment) : 0,
                    ...
                    // Manual adjustment flag (not in cloud but needed locally)
                ...
                // DEBUG: Log the complete entry being saved
                if (typeof window.addDebugLog === 'function') {
                    window.addDebugLog(`db.js: Saving entry with adjustments: al=${completeEntry.al_adjustment}, sl=${completeEntry.sl_adjustment}, cl=${completeEntry.cl_adjustment}, cpl=${completeEntry.cpl_adjustment}, ot=${completeEntry.ot_adjustment}`, 'debug');
                }
                ...
                request.onsuccess = () => {
                    if (typeof window.addDebugLog === 'function') {
                        ...
                        window.addDebugLog(`db.js: Saved with al_adjustment=${completeEntry.al_adjustment}, sl_adjustment=${completeEntry.sl_adjustment}, cl_adjustment=${completeEntry.cl_adjustment}, cpl_adjustment=${completeEntry.cpl_adjustment}, ot_adjustment=${completeEntry.ot_adjustment}`, 'info');
                    }
            ...
            try {
                ...
                request.onsuccess = () => {
                    ...
                    if (typeof window.addDebugLog === 'function') {
                        ...
                        // Log first few entries to debug adjustment fields
                        userEntries.slice(0, 3).forEach(e => {
                            window.addDebugLog(`db.js: Entry ${e.date}: al=${e.al_adjustment}, sl=${e.sl_adjustment}, cl=${e.cl_adjustment}, cpl=${e.cpl_adjustment}, ot=${e.ot_adjustment}`, 'debug');
                        });
                    }
                ...
                if (store.indexNames.contains('sync_status')) {
                    ...
                    request.onsuccess = () => {
                        ...
                        if (typeof window.addDebugLog === 'function') {
                            ...
                            // Log adjustment values for pending entries
                            limited.forEach(e => {
                                window.addDebugLog(`db.js: Pending ${e.date}: al=${e.al_adjustment}, sl=${e.sl_adjustment}, cl=${e.cl_adjustment}, cpl=${e.cpl_adjustment}, ot=${e.ot_adjustment}`, 'debug');
                            });
                        }
