...
(function() {
    ...
    function showOTPModal(otpData, purpose, email) {
        ...
        window.addDebugLog('OTP modal shown for purpose: ' + purpose, 'info');
    }
    ...
    function startOTPTimer(expiryDate) {
        ...
        function updateTimer() {
            ...
            if (diff <= 0) {
                ...
                window.addDebugLog('OTP expired', 'warning');
                return;
            }
    ...
    function copyToClipboard(elementId) {
        ...
        navigator.clipboard.writeText(text).then(() => {
            window.addDebugLog(`Copied to clipboard: ${text}`, 'success');
            alert('Copied to clipboard! (OTP is case-insensitive)');
        }).catch(err => {
            window.addDebugLog('Failed to copy: ' + err, 'error');
    ...
    function openMailClient() {
        ...
        window.addDebugLog('Opened email client', 'info');
    }
    ...
    async function verifyOTP() {
        ...
        try {
            ...
            if (data.success) {
                window.addDebugLog('OTP verified successfully', 'success');
                closeOTPModal();
                ...
                window.addDebugLog('OTP verification failed: ' + data.message, 'error');
                document.getElementById('verifyOtpBtn').disabled = false;
            ...
            window.addDebugLog('OTP verification error: ' + error, 'error');
            document.getElementById('verifyOtpBtn').disabled = false;
            document.getElementById('verifyOtpBtn').textContent = "I've Sent the Email";
        }
    }
    ...
    async function initiateForgotPassword(email) {
        window.addDebugLog('Initiating password reset for: ' + email, 'info');
        ...
        try {
            ...
            window.addDebugLog('Forgot password error: ' + error, 'error');
    ...
    function showResetPasswordModal(email) {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 400px;">
                ...
                <p style="color: #666; margin-bottom: 20px;">Enter your new password for ${email}</p>
                
                <div class="input-group">
                    <label>New Password</label>
                    <input type="password" id="resetNewPassword" class="modal-input" placeholder="Min 6 characters">
                </div>
                
                <div class="input-group">
                    <label>Confirm Password</label>
                    <input type="password" id="resetConfirmPassword" class="modal-input" placeholder="Re-enter new password">
                </div>
                
                <div id="resetError" class="error-message"></div>
                
                <div class="modal-buttons">
                    <button onclick="submitPasswordReset('${email}')" class="modal-save">Reset Password</button>
                    <button onclick="closeResetModal()" class="modal-cancel">Cancel</button>
                </div>
            </div>
        `;
    ...
    async function submitPasswordReset(email) {
        ...
        try {
            ...
            window.addDebugLog('Password reset error: ' + error, 'error');
    ...
    async function markLeave(type) {
        ...
        // Clear any existing work/leave data
        entry.check_in = null;
        entry.check_out = null;
        ...
        // Reset all leave usage to 0 first
        entry.al_used = 0;
        entry.sl_used = 0;
        entry.cl_used = 0;
        entry.cpl_used = 0;
        
        // Set the specific leave type to 1
        entry[`${type}_used`] = 1;
    ...
    // ==================== BALANCE ADJUSTMENT FUNCTIONS - WITH PROPER EXPIRY ====================
    ...
    async function saveBalanceAdjustment() {
        ...
        // Store adjustments in separate columns and ALWAYS recalculate expiry dates
        if (al !== 0) {
            ...
            // ALWAYS set expiry date for positive AL adjustments
            if (entry.al_adjustment > 0) {
                ...
                // If adjustment is zero or negative, clear expiry date
        ...
        if (cpl !== 0) {
            ...
            // ALWAYS set expiry date for positive CPL adjustments
    ...
    async function saveInitialBalances() {
        ...
        // Store as adjustments - ALWAYS set expiry dates for positive adjustments
        if (al !== 0) {
            entry.al_adjustment = (entry.al_adjustment || 0) + al;
    ...
    window.addDebugLog('app.js: Loading complete - ULTRA DEBUG VERSION WITH OTP', 'success');
})();