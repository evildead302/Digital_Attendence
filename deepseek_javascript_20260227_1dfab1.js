...
(function() {
    ...
    async function openUserDatabase(userId) {
        ...
        return new Promise((resolve, reject) => {
            const request = indexedDB.open(dbName, 3); // Increment to version 3 for new fields
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Create entries store with all fields including adjustments
                if (!db.objectStoreNames.contains('entries')) {
    ...
    // Save entry with optimized write - UPDATED to include adjustment fields
    async function saveEntry(entry) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                // Ensure entry has all required fields including adjustments
                const completeEntry = {
                    // Core fields
                    date: entry.date,
    ...
    // Get all entries for user with 97-day window - UPDATED to include adjustments
            ...
            try {
                ...
                request.onsuccess = () => {
                    ...
                    const stats = {
                        ...
                        adjustments: userEntries.filter(e => 
                            e.al_adjustment !== 0 || 
                            e.sl_adjustment !== 0 || 
                            e.cl_adjustment !== 0 || 
                            e.cpl_adjustment !== 0 || 
