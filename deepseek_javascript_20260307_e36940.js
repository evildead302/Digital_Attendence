...
(function() {
    ...
    async function openUserDatabase(userId) {
        ...
        return new Promise((resolve, reject) => {
            const request = indexedDB.open(dbName, 6); // Version 6 for adjustment fields
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Create entries store with all fields
                if (!db.objectStoreNames.contains('entries')) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const completeEntry = {
                    ...
                    // Adjustment fields - CRITICAL: These must be saved
                    al_adjustment: entry.al_adjustment !== undefined ? entry.al_adjustment : 0,
                    cpl_adjustment: entry.cpl_adjustment !== undefined ? entry.cpl_adjustment : 0,
                    ot_adjustment: entry.ot_adjustment !== undefined ? entry.ot_adjustment : 0,
                ...
                request.onsuccess = () => {
                    if (typeof window.addDebugLog === 'function') {
                        ...
                        window.addDebugLog(`db.js: Saved with al_adjustment=${completeEntry.al_adjustment}, cpl_adjustment=${completeEntry.cpl_adjustment}, ot_adjustment=${completeEntry.ot_adjustment}`, 'info');
                    }
