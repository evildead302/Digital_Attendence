...
(function() {
    ...
    // Initiate forgot password flow - UPDATED ENDPOINT
    ...
    // Submit password reset - UPDATED ENDPOINT
    async function submitPasswordReset(email) {
        ...
        if (!newPassword || newPassword.length < 6) {
            ...
            document.getElementById('resetError').textContent = errorMsg;
            return;
        }
    ...
    async function register() {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                window.addDebugLog(`Registration HTTP error: ${response.status} - ${errorText}`, 'error');
                throw new Error(`HTTP ${response.status}`);
            }
    ...
    // UPDATED DELETE ACCOUNT ENDPOINT
    ...
    async function resetAllData() {
        ...
        try {
            // UPDATED: Use account.js endpoint for reset-data
